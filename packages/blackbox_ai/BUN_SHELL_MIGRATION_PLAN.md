...
Replace runtime Bun shell template-tag usage in `packages/blackbox ai/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/blackbox ai/src` is removed except:
