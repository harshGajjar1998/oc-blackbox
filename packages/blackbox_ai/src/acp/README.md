...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for blackbox ai.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal blackbox ai sessions
...
blackbox ai acp --cwd /path/to/project
```
...
```bash
BLACKBOX_AI_ENABLE_QUESTION_TOOL=1 blackbox ai acp
```
...
- **Terminal Integration**: Full terminal support via blackbox ai's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | blackbox ai acp
```
...
### Mapping to Blackbox AI

ACP sessions map cleanly to blackbox ai's internal session model:
