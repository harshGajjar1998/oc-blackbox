...
export namespace Config {
  ...
  export const state = Instance.state(async () => {
    ...
    // Config loading order (low -> high precedence): https://www.blackbox.ai/docs/config#precedence-order
    ...
    for (const [key, value] of Object.entries(auth)) {
      if (value.type === "wellknown") {
        ...
        if (!remoteConfig.$schema) remoteConfig.$schema = "https://www.blackbox.ai/config.json"
        result = merge(
          result,
  ...
  export const Info = z
    .object({
      ...
      command: z
        ...
        .optional()
        .describe("Command configuration, see https://www.blackbox.ai/docs/commands"),
      ...
      agent: z
        ...
        .catchall(Agent)
        .optional()
        .describe("Agent configuration, see https://www.blackbox.ai/docs/agents"),
      provider: z
    ...
    if (existsSync(legacy)) {
      await import(pathToFileURL(legacy).href, {
        ...
        .then(async (mod) => {
          ...
          result["$schema"] = "https://www.blackbox.ai/config.json"
  ...
  async function load(text: string, options: { path: string } | { dir: string; source: string }) {
    ...
    if (parsed.success) {
      if (!parsed.data.$schema && isFile) {
        parsed.data.$schema = "https://www.blackbox.ai/config.json"
