...
export namespace Provider {
  ...
  const CUSTOM_LOADERS: Record<string, CustomLoader> = {
    ...
    async opencode(input) {
      ...
      const baseURL =
        config.provider?.["opencode"]?.options?.baseURL ?? Env.get("BLACKBOX_API_BASE_URL") ?? "https://api.blackbox.ai"
  ...
  const state = Instance.state(async () => {
    ...
    const configProviders = Object.entries(config.provider ?? {}).filter(([providerID]) => providerID === "opencode")
