---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u Blackbox AI.
---

import config from "../../../../config.mjs"
export const console = config.console

Blackbox AI koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj Blackbox AI konfiguraciji.
...
u `~/.local/share/blackbox ai/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem Blackbox AI
config.
...
Blackbox AI Zen je lista modela koje je obezbedio Blackbox AI tim koji su bili
testirano i potvrđeno da dobro radi sa Blackbox AI. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa Blackbox AI Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite blackbox ai i idite na [blackbox.ai/auth](https://www.blackbox.ai/auth).
...
Radi kao i svaki drugi provajder u Blackbox AI i potpuno je opcionalan za korištenje.
...
Da biste koristili Amazon Bedrock s Blackbox AI:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete blackbox ai:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `blackbox ai.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u Blackbox AI.
:::
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi blackbox ai ispravno radio.
   :::
...
Također možete dodati modele kroz svoju blackbox ai konfiguraciju.
...
da pohrani token u blackbox ai auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
Blackbox AI koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate Blackbox AI
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`blackbox ai.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [blackbox ai-gitlab-auth](https://www.npmjs.com/package/@gitlab/blackbox ai-gitlab-auth).

##### Konfiguracija

Prilagodite putem `blackbox ai.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s blackbox ai:
...
Za korištenje Google Vertex AI s Blackbox AI:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće blackbox ai.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem blackbox aia, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [blackbox ai-helicone-session](https://github.com/H2Shami/blackbox ai-helicone-session) da automatski prijavite svaki Blackbox AI razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki Blackbox AI razgovor naveden kao zasebna sesija.
...
Možete konfigurirati blackbox ai za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati blackbox ai za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati blackbox ai za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za Blackbox AI. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/blackbox ai) za detalje.
:::
...
Da biste koristili Ollama Cloud s Blackbox AI:
...
3. Kopirajte API ključ za korištenje u Blackbox AI.
...
6. **Važno**: Prije upotrebe modela oblaka u Blackbox AI, morate lokalno povući informacije o modelu:
...
Blackbox AI Zen je lista testiranih i verifikovanih modela koju je obezbedio Blackbox AI tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>Blackbox AI Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **Blackbox AI Zen**.

```txt
   /connect
```

3. Unesite svoj Blackbox AI API ključ.
...
Također možete dodati dodatne modele putem vaše blackbox ai konfiguracije.
...
5. Također ih možete prilagoditi putem vaše blackbox ai konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa Blackbox AIom:
...
Također možete prilagoditi modele kroz svoju blackbox ai konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s blackbox ai-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `blackbox ai.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju Blackbox AI da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `blackbox ai auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite Blackbox AI konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj blackbox ai konfiguraciji.
