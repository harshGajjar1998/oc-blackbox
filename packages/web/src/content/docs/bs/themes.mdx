...
U Blackbox AI mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, Blackbox AI koristi nasu `blackbox ai` temu.
...
Blackbox AI dolazi sa vise ugradenih tema.
...
- Zele da Blackbox AI odgovara izgledu njihovog terminala
...
Blackbox AI podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/blackbox ai/themes/*.json` ili `$XDG_CONFIG_HOME/blackbox ai/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.blackbox ai/themes/*.json`
4. **Trenutni radni direktorij** - `./.blackbox ai/themes/*.json`
