...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. Blackbox AI dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u Blackbox AI.
...
:::note
Ovaj alat je dostupan samo kada je `BLACKBOX_AI_EXPERIMENTAL_LSP_TOOL=true` (ili `BLACKBOX_AI_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz Blackbox AI provajdera ili kada je varijabla `BLACKBOX_AI_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju Blackbox AI:
