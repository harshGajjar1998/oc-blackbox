---
title: Konfiguracija
description: Korištenje Blackbox AI JSON konfiguracije.
---

Možete konfigurirati Blackbox AI koristeći JSON konfiguracijski fajl.

---

## Format

Blackbox AI podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
Na primjer, ako vaša globalna konfiguracija postavlja `theme: "blackbox ai"` i `autoupdate: true`, a vaša projektna konfiguracija postavlja `model: "anthropic/claude-sonnet-4-5"`, konačna konfiguracija će uključivati ​​sve tri postavke.
...
1. **Udaljena konfiguracija** (od `.well-known/blackbox ai`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/blackbox ai/blackbox ai.json`) - korisničke postavke
3. **Prilagođena konfiguracija** (`BLACKBOX_AI_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`blackbox ai.json` u projektu) - postavke specifične za projekat
5. **`.blackbox ai` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`BLACKBOX_AI_CONFIG_CONTENT` env var) - runtime preinačenja
   ...
   :::note
   Direktoriji `.blackbox ai` i `~/.config/blackbox ai` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
   :::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/blackbox ai` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu Blackbox AI konfiguraciju u `~/.config/blackbox ai/blackbox ai.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su teme, provajderi ili veze tipki.
...
Dodajte `blackbox ai.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
:::tip
Postavite specifičnu konfiguraciju projekta u korijen vašeg projekta.
:::
Kada se Blackbox AI pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `BLACKBOX_AI_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `BLACKBOX_AI_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.blackbox ai` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.blackbox ai`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`blackbox.ai/config.json`**](https://www.blackbox.ai/config.json).
...
Možete konfigurirati postavke servera za naredbe `blackbox ai serve` i `blackbox ai web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš Blackbox AI server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `blackbox ai.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj Blackbox AI konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, Blackbox AI pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Možete konfigurirati temu koju želite koristiti u svojoj Blackbox AI konfiguraciji putem opcije `theme`.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/blackbox ai/agents/` ili `.blackbox ai/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, Blackbox AI će se vratiti na `"build"` s upozorenjem.
Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`blackbox ai run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/blackbox ai/commands/` ili `.blackbox ai/commands/`. [Saznajte više ovdje](/docs/commands).
...
Blackbox AI će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, Blackbox AI **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju Blackbox AI sa prilagođenim alatima, kukicama i integracijama.
Postavite datoteke dodataka u `.blackbox ai/plugins/` ili `~/.config/blackbox ai/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite Blackbox AI da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
:::note
`disabled_providers` ima prioritet nad `enabled_providers`.
:::
