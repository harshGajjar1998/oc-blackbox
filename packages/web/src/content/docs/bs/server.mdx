---
title: Server
description: Komunicirajte s blackbox ai serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `blackbox ai serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji blackbox ai klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `blackbox ai.local` |
...
Postavite `BLACKBOX_AI_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `blackbox ai`, ili postavite `BLACKBOX_AI_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `blackbox ai serve` i za `blackbox ai web`.
...
Kada pokrenete `blackbox ai`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite blackbox ai server za programsku interakciju sa blackbox ai-om.
:::

Ova arhitektura omogucava blackbox ai podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `blackbox ai serve` da startate standalone server. Ako je blackbox ai TUI vec pokrenut, `blackbox ai serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste Blackbox AI [IDE](/docs/ide) pluginovi.
...
blackbox ai server izlaže sljedece API-je.
