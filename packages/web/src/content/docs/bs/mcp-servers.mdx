...
Možete dodati vanjske alate u Blackbox AI koristeći _Model Context Protocol_, ili MCP. Blackbox AI podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [Blackbox AI Config](https://www.blackbox.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/blackbox ai`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
Blackbox AI automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, Blackbox AI će:
...
Ako server zahtijeva autentifikaciju, Blackbox AI će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `blackbox ai mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, Blackbox AI sigurno čuva tokene u `~/.local/share/blackbox ai/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u Blackbox AI, zajedno s ugrađenim alatima. Možete njima upravljati kroz Blackbox AI konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete Blackbox AI sa vašim Sentry nalogom.
