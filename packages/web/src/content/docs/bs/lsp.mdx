---
title: LSP serveri
description: Blackbox AI se integriše sa vašim LSP serverima.
---

Blackbox AI se integriše sa vašim Language Server Protocol (LSP) serverima kako bi pomogao LLM-u u interakciji s kodnom bazom. Koristi dijagnostiku za slanje povratnih informacija modelu.

## Ugrađeni

Blackbox AI dolazi sa nekoliko ugrađenih LSP servera za popularne jezike:
...
:::note
Možete onemogućiti automatska preuzimanja LSP servera tako što ćete postaviti varijablu okruženja `BLACKBOX_AI_DISABLE_LSP_DOWNLOAD` na `true`.
:::
...
Kada blackbox ai otvori fajl, on:
...
Možete prilagoditi LSP servere kroz `lsp` odjeljak u vašoj blackbox ai konfiguraciji.
