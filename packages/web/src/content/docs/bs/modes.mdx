...
ove ili konfigurirajte svoje putem blackbox ai config.
...
blackbox ai dolazi sa dva ugrađena načina rada.
...
- `edit` - Ne mogu modificirati postojeće fajlove, osim fajlova koji se nalaze na `.blackbox ai/plans/*.md` radi detaljiziranja samog plana
...
Konfigurirajte načine rada u svom `blackbox ai.json` konfiguracijskom fajlu:
...
- Globalno: `~/.config/blackbox ai/modes/`
- Projekat: `.blackbox ai/modes/`
...
Ako temperatura nije navedena, blackbox ai koristi podrazumijevane postavke specifične za model (obično 0 za većinu modela i 0.55 za Qwen modele).
...
Kreirajte fajlove načina u `.blackbox ai/modes/` za specifične načine rada ili `~/.config/blackbox ai/modes/` za globalne načine:
