---
title: Dodaci
description: Napišite vlastite dodatke za proširenje Blackbox AI.
---

Dodaci vam omogućavaju da proširite Blackbox AI spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja Blackbox AI.
...
- `.blackbox ai/plugins/` - Dodaci na nivou projekta
- `~/.config/blackbox ai/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/blackbox ai/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/blackbox ai/blackbox ai.json`)
2. Konfiguracija projekta (`blackbox ai.json`)
3. Globalni direktorij dodataka (`~/.config/blackbox ai/plugins/`)
4. Direktorij dodataka projekta (`.blackbox ai/plugins/`)
...
Blackbox AI pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: Blackbox AI SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje Blackbox AI.
...
:::note
Ako koristite desktop aplikaciju Blackbox AI, ona može automatski slati sistemske obavijesti kada je odgovor spreman ili kada dođe do greške u sesiji.
:::
...
Spriječite blackbox ai da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u blackbox ai:
...
Pomoćnik `tool` kreira prilagođeni alat koji blackbox ai može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za blackbox ai zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://www.blackbox.ai/docs/sdk) za detalje.
