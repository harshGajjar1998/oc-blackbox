---
title: المزوّدون
description: استخدام أي موفّر LLM في Blackbox AI.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم Blackbox AI ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات Blackbox AI.
...
في `~/.local/share/blackbox ai/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات Blackbox AI.
...
Blackbox AI Zen هي قائمة نماذج يوفّرها فريق Blackbox AI وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع Blackbox AI. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع Blackbox AI Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر blackbox ai، ثم انتقل إلى [blackbox.ai/auth](https://www.blackbox.ai/auth).
...
يعمل مثل أي مزوّد آخر في Blackbox AI واستخدامه اختياري بالكامل.
...
لاستخدام Amazon Bedrock مع Blackbox AI:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل blackbox ai:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `blackbox ai.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل Blackbox AI غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل blackbox ai بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات blackbox ai.
...
تخزين الرمز في مساحة تخزين مصادقة blackbox ai.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم Blackbox AI نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر Blackbox AI
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`blackbox ai.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [blackbox ai-gitlab-auth](https://www.npmjs.com/package/@gitlab/blackbox ai-gitlab-auth).

##### التهيئة

خصّص عبر `blackbox ai.json`:
...
لاستخدام اشتراك GitHub Copilot مع blackbox ai:
...
لاستخدام Google Vertex AI مع Blackbox AI:
...
2. عيّن متغيرات البيئة المطلوبة:
   ...
   عيّنها أثناء تشغيل blackbox ai.
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر blackbox ai، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [blackbox ai-helicone-session](https://github.com/H2Shami/blackbox ai-helicone-session) لتسجيل كل محادثة Blackbox AI تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة Blackbox AI مدرجة كجلسة مستقلة.
...
يمكنك ضبط blackbox ai لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط blackbox ai لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط blackbox ai لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع Blackbox AI. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/blackbox ai) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع Blackbox AI:
...
3. انسخ مفتاح API لاستخدامه في Blackbox AI.
...
6. **مهم**: قبل استخدام نماذج السحابة في Blackbox AI، يجب سحب معلومات النموذج محليا:
...
Blackbox AI Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق Blackbox AI. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>Blackbox AI Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **Blackbox AI Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ Blackbox AI.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات blackbox ai.
...
5. يمكنك أيضا تخصيصها عبر إعدادات blackbox ai. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع Blackbox AI:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات blackbox ai. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع blackbox ai. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `blackbox ai.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ Blackbox AI بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `blackbox ai auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات blackbox ai وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات blackbox ai.
