...
مع Blackbox AI يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم Blackbox AI سمتنا `blackbox ai`.
...
يأتي Blackbox AI مع عدة سمات مدمجة.
...
- يريدون أن يطابق Blackbox AI مظهر terminal لديهم
...
يدعم Blackbox AI نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/blackbox ai/themes/*.json` أو `$XDG_CONFIG_HOME/blackbox ai/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.blackbox ai/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.blackbox ai/themes/*.json`
