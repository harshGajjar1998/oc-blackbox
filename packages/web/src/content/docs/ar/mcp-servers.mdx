...
يمكنك إضافة أدوات خارجية إلى Blackbox AI باستخدام _Model Context Protocol_، أو MCP. يدعم Blackbox AI خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات Blackbox AI](https://www.blackbox.ai/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يمكن للمنظمات توفير خوادم MCP الافتراضية عبر نقطة النهاية `.well-known/blackbox ai` الخاصة بها. قد تكون هذه الخوادم معطّلة افتراضيا، مما يتيح للمستخدمين تفعيل ما يحتاجونه فقط.
...
يتولى Blackbox AI تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم Blackbox AI بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك Blackbox AI المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `blackbox ai mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن Blackbox AI الرموز بشكل آمن في `~/.local/share/blackbox ai/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل Blackbox AI إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات Blackbox AI مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط Blackbox AI بحساب Sentry الخاص بك.
