...
تتيح مهارات الوكيل لـ Blackbox AI اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث Blackbox AI في هذه المواقع:

- إعدادات المشروع: `.blackbox ai/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/blackbox ai/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج Blackbox AI صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.blackbox ai/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/blackbox ai/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.blackbox ai/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض Blackbox AI المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `blackbox ai.json`:
...
**للوكلاء المدمجين** (في `blackbox ai.json`):
