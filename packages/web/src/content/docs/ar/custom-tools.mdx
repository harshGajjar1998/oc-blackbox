...
description: أنشئ أدوات يمكن لـ LLM استدعاؤها في blackbox ai.
---

الأدوات المخصصة هي دوال تنشئها يمكن لـ LLM استدعاؤها أثناء المحادثات. تعمل جنبا إلى جنب مع [الأدوات المدمجة](/docs/tools) في blackbox ai مثل `read` و`write` و`bash`.
...
- محليا بوضعها في مجلد `.blackbox ai/tools/` داخل مشروعك.
- أو عالميا بوضعها في `~/.config/blackbox ai/tools/`.
