---
title: الخادم
description: تفاعل مع خادم blackbox ai عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `blackbox ai serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل blackbox ai استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `blackbox ai.local` |
...
عيّن `BLACKBOX_AI_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `blackbox ai`، أو عيّن `BLACKBOX_AI_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `blackbox ai serve` و `blackbox ai web`.
...
عند تشغيل `blackbox ai` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم blackbox ai للتفاعل مع blackbox ai برمجيا.
:::

تتيح هذه البنية لـ blackbox ai دعم عدة عملاء وتمكّنك من التفاعل مع blackbox ai برمجيا.

يمكنك تشغيل `blackbox ai serve` لبدء خادم مستقل. إذا كانت واجهة blackbox ai في terminal (TUI)
قيد التشغيل، فسيبدأ `blackbox ai serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات Blackbox AI لـ [IDE](/docs/ide).
...
يعرض خادم blackbox ai واجهات API التالية.
