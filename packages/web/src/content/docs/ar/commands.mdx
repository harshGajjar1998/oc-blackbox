...
أنشئ الملف `.blackbox ai/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات Blackbox AI أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ Blackbox AI:
...
- على مستوى النظام: `~/.config/blackbox ai/commands/`
- لكل مشروع: `.blackbox ai/commands/`
...
يتضمن blackbox ai عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
