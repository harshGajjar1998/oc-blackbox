...
:::caution
يتم الآن ضبط الأوضاع عبر خيار `agent` في إعدادات blackbox ai. أصبح خيار
`mode` مُهمَلًا الآن. [اعرف المزيد](/docs/agents).
:::

تتيح لك الأوضاع في blackbox ai تخصيص السلوك والأدوات والمطالبات لحالات استخدام مختلفة.

يأتي مع وضعين مدمجين: **build** و **plan**. يمكنك تخصيصهما أو إعداد
أوضاعك الخاصة عبر إعدادات blackbox ai.
...
يأتي blackbox ai مع وضعين مدمجين.
...
- `edit` - لا يمكن تعديل الملفات الموجودة، باستثناء الملفات الموجودة في `.blackbox ai/plans/*.md` لتفصيل الخطة نفسها
...
قم بإعداد الأوضاع في ملف الإعدادات `blackbox ai.json` لديك:
...
- عالمي: `~/.config/blackbox ai/modes/`
- للمشروع: `.blackbox ai/modes/`
...
إذا لم يتم تحديد درجة الحرارة، يستخدم blackbox ai القيم الافتراضية الخاصة بكل نموذج (عادةً 0 لمعظم النماذج و 0.55 لنماذج Qwen).
...
إعدادات blackbox ai العالمية وكذلك إعدادات المشروع.
...
أنشئ ملفات الأوضاع في `.blackbox ai/modes/` لأوضاع خاصة بالمشروع أو في `~/.config/blackbox ai/modes/` لأوضاع عالمية:
