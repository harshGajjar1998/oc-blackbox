---
title: Server
description: Interact with blackbox ai server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `blackbox ai serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an blackbox ai client can use.
...
Set `BLACKBOX_AI_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `blackbox ai`, or set `BLACKBOX_AI_SERVER_USERNAME` to override it. This applies to both `blackbox ai serve` and `blackbox ai web`.
...
When you run `blackbox ai` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the blackbox ai server to interact with blackbox ai programmatically.
:::

This architecture lets blackbox ai support multiple clients and allows you to interact with blackbox ai programmatically.

You can run `blackbox ai serve` to start a standalone server. If you have the
blackbox ai TUI running, `blackbox ai serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Blackbox AI [IDE](/docs/ide) plugins.
...
The blackbox ai server exposes the following APIs.
