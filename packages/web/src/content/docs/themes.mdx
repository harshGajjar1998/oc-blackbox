...
With Blackbox AI you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Blackbox AI uses our own `blackbox ai` theme.
...
Blackbox AI comes with several built-in themes.
...
Blackbox AI supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/blackbox ai/themes/*.json` or `$XDG_CONFIG_HOME/blackbox ai/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.blackbox ai/themes/*.json`
4. **Current working directory** - Defined in `./.blackbox ai/themes/*.json`
