...
blackbox ai'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
blackbox ai, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
blackbox ai, **General** ve **Explore** olmak üzere iki yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

blackbox ai iki yerleşik birincil agent ve iki yerleşik alt agent ile birlikte gelir.
...
Agent'ları `blackbox ai.json` yapılandırma dosyanızda yapılandırın:
...
- Per-project: `.blackbox ai/agents/`
...
Sıcaklık belirtilmezse blackbox ai modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global blackbox ai yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
blackbox ai hesabınızdaki model kimliğini `provider/model-id` biçimini kullanır. Örneğin, [Blackbox AI Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `blackbox ai/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `blackbox ai models` komutunu çalıştırın.
:::
...
:::tip
Paylaşmak istediğiniz bir agent'ınız var mı? [Submit a PR](https://github.com/anomalyco/blackbox ai).
:::
