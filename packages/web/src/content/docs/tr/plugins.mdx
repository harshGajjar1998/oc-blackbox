---
title: Eklentiler
description: blackbox ai'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek blackbox ai'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya blackbox ai'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.blackbox ai/plugins/` - Proje düzeyinde eklentiler
- `~/.config/blackbox ai/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/blackbox ai/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/blackbox ai/blackbox ai.json`)
2. Project config (`blackbox ai.json`)
3. Global eklenti dizini (`~/.config/blackbox ai/plugins/`)
4. Proje eklenti dizini (`.blackbox ai/plugins/`)
...
blackbox ai bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşime geçmek için blackbox ai'lu bir SDK istemcisi.
...
blackbox ai'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
blackbox ai masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
blackbox ai'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca blackbox ai'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, blackbox ai'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK documentation](https://www.blackbox.ai/docs/sdk)'e bakın.
