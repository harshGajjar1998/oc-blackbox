...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. blackbox ai yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
blackbox ai içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `BLACKBOX_AI_EXPERIMENTAL_LSP_TOOL=true` (veya `BLACKBOX_AI_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca blackbox ai provider kullanılırken veya `BLACKBOX_AI_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

blackbox ai başlatırken etkinleştirmek için:
