---
title: Sunucu
description: blackbox ai sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`blackbox ai serve` komutu, blackbox ai istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `blackbox ai.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `BLACKBOX_AI_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `blackbox ai` değeridir; değiştirmek isterseniz `BLACKBOX_AI_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `blackbox ai serve` hem de `blackbox ai web` için geçerlidir.
...
`blackbox ai` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
blackbox ai ile programatik etkileşim için blackbox ai sunucusunu kullanın.
:::

Bu mimari, blackbox ai'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`blackbox ai serve` ile bağımsız bir sunucu başlatabilirsiniz. blackbox ai TUI açıksa, `blackbox ai serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum blackbox ai [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
blackbox ai sunucusu aşağıdaki API'leri sunar.
