---
title: Sağlayıcılar
description: blackbox ai'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

blackbox ai, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı blackbox ai yapılandırmanızda yapılandırın.
...
`~/.local/share/blackbox ai/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları blackbox ai'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
Blackbox AI Zen, blackbox ai ekibi tarafından sağlanan modellerin bir listesidir.
blackbox ai ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz Blackbox AI Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, blackbox ai'u seçin ve [blackbox.ai/auth](https://www.blackbox.ai/auth)'ye gidin.
...
blackbox ai'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.
...
Amazon Bedrock'u blackbox ai ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   blackbox ai'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `blackbox ai.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi blackbox ai'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   blackbox ai'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
5. Bir model seçmek için `/models` komutunu çalıştırın.
   ...
   Blackbox AI yapılandırmanız aracılığıyla da modeller ekleyebilirsiniz.
...
to store token in blackbox ai auth storage.
:::

##### Kendi Kendine Barındırılan GitLab

:::note[uyumluluk notu]
blackbox ai, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. blackbox ai'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`blackbox ai.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [blackbox ai-gitlab-auth](https://www.npmjs.com/package/@gitlab/blackbox ai-gitlab-auth) ana sayfasında.

##### Yapılandırma

`blackbox ai.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi blackbox ai'la kullanmak için:
...
Google Vertex AI'yi blackbox ai ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları blackbox ai çalıştırılırken ayarlayın.
...
Helicone'un blackbox ai aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her blackbox ai görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [blackbox ai-helicone-session](https://github.com/H2Shami/blackbox ai-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her blackbox ai görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
blackbox ai'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla blackbox ai'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla blackbox ai'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini blackbox ai için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/blackbox ai)'a bakın.
:::
...
Ollama Cloud'u blackbox ai ile kullanmak için:
...
3. blackbox ai'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: blackbox ai'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
Blackbox AI Zen, blackbox ai ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>Blackbox AI Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **Blackbox AI Zen**'i arayın.

   ```txt
   /connect
   ```

3. blackbox ai API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   blackbox ai yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca blackbox ai yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)'yi Blackbox AI ile kullanmak için:
...
Modelleri blackbox ai yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı blackbox ai ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `blackbox ai.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları blackbox ai'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `blackbox ai auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için blackbox ai yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, blackbox ai yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
