...
_Model Bağlam Protokolü_ veya MCP kullanarak blackbox ai'a harici araçlar ekleyebilirsiniz. blackbox ai hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [blackbox ai Config](https://www.blackbox.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/blackbox ai` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
blackbox ai, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde blackbox ai şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa blackbox ai, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `blackbox ai mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından blackbox ai, belirteçleri `~/.local/share/blackbox ai/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra blackbox ai'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi blackbox ai yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve blackbox ai'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
