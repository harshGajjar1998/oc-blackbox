...
Ajan becerileri, blackbox ai'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
blackbox ai su konumlari tarar:

- Proje konfigurasyonu: `.blackbox ai/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/blackbox ai/skills/<name>/SKILL.md`
...
Proje yerel yollarinda blackbox ai, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.blackbox ai/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/blackbox ai/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.blackbox ai/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
blackbox ai, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`blackbox ai.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`blackbox ai.json` icinde):
