...
:::caution
Modlar artık blackbox ai yapılandırmasındaki `agent` seçeneği aracılığıyla yapılandırılıyor.
`mode` seçeneği artık kullanımdan kaldırıldı. [Daha fazla bilgi](/docs/agents).
:::

blackbox ai'daki modlar, farklı kullanım durumları için davranışı, araçları ve istemleri özelleştirmenize olanak tanır.

İki yerleşik modla birlikte gelir: **build** ve **plan**. Kişiselleştirebilirsiniz
bunları veya blackbox ai yapılandırması aracılığıyla kendinizinkini yapılandırın.
...
blackbox ai iki yerleşik modla birlikte gelir.
...
- `edit` - Planın kendisini detaylandırmak için `.blackbox ai/plans/*.md` adresinde bulunan dosyalar dışında mevcut dosyalar değiştirilemez
...
`blackbox ai.json` yapılandırma dosyanızdaki modları yapılandırın:
...
Sıcaklık belirtilmezse, blackbox ai modele özgü varsayılanları kullanır (çoğu model için genellikle 0, Qwen modelleri için 0,55).
...
hem global blackbox ai yapılandırması hem de projeye özel yapılandırma.
...
Projeye özel modlar için `.blackbox ai/modes/` veya genel modlar için `~/.config/blackbox ai/modes/`'de mod dosyaları oluşturun:
