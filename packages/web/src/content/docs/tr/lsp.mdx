...
description: blackbox ai, LSP sunucularınızla bütünleşir.
---

blackbox ai, LLM'in kod tabanınızla etkileşime girmesine yardımcı olmak için Dil Sunucusu Protokolünüzle (LSP) bütünleşir. LLM'ye geri bildirim sağlamak için tanılamayı kullanır.

---

## Yerleşik

blackbox ai, popüler diller için çeşitli yerleşik LSP sunucularıyla birlikte gelir:
...
:::note
`BLACKBOX_AI_DISABLE_LSP_DOWNLOAD` ortam değişkenini `true` olarak ayarlayarak otomatik LSP sunucu indirmelerini devre dışı bırakabilirsiniz.
:::
...
Blackbox AI bir dosyayı açtığında:
...
LSP sunucularını, blackbox ai yapılandırmanızdaki `lsp` bölümü aracılığıyla özelleştirebilirsiniz.
