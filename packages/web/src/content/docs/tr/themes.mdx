...
blackbox ai ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak blackbox ai kendi `blackbox ai` temasını kullanır.
...
blackbox ai birden fazla yerleşik temayla gelir.
...
- blackbox ai'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
blackbox ai, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/blackbox ai/themes/*.json` veya `$XDG_CONFIG_HOME/blackbox ai/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.blackbox ai/themes/*.json`
4. **Geçerli çalışma dizini** - `./.blackbox ai/themes/*.json`
