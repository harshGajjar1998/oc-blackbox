---
title: LSP Servers
description: Blackbox AI integrates with your LSP servers.
---

Blackbox AI integrates with your Language Server Protocol (LSP) to help the LLM interact with your codebase. It uses diagnostics to provide feedback to the LLM.

---

## Built-in

Blackbox AI comes with several built-in LSP servers for popular languages:
...
:::note
You can disable automatic LSP server downloads by setting the `BLACKBOX_AI_DISABLE_LSP_DOWNLOAD` environment variable to `true`.
:::
...
When blackbox ai opens a file, it:
...
You can customize LSP servers through the `lsp` section in your blackbox ai config.
