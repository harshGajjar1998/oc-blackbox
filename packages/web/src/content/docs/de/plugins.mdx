---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um Blackbox AI zu erweitern.
---

Mit Plugins können Sie Blackbox AI erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von Blackbox AI zu ändern.
...
- `.blackbox ai/plugins/` – Plugins auf Projektebene
- `~/.config/blackbox ai/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/blackbox ai/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/blackbox ai/blackbox ai.json`)
2. Projektkonfiguration (`blackbox ai.json`)
3. Globales Plugin-Verzeichnis (`~/.config/blackbox ai/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.blackbox ai/plugins/`)
...
Blackbox AI führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein Blackbox AI-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie Blackbox AI erweitern können.
...
:::note
Wenn Sie die Blackbox AI-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass Blackbox AI `.env`-Dateien liest:
...
Plugins können Blackbox AI auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das Blackbox AI aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für Blackbox AI zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://www.blackbox.ai/docs/sdk).
