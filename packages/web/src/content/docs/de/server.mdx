---
title: Server
description: Nutze den blackbox ai-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `blackbox ai serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein blackbox ai-Client nutzen kann.
...
Setze `BLACKBOX_AI_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `blackbox ai`, kann aber mit `BLACKBOX_AI_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `blackbox ai serve` und `blackbox ai web`.
...
Wenn du `blackbox ai` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den blackbox ai-Server, um blackbox ai programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `blackbox ai serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `blackbox ai serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die Blackbox AI-[IDE](/docs/ide)-Plugins.
...
Der blackbox ai-Server stellt folgende APIs bereit.
