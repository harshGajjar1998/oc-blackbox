---
title: Anbieter
description: Verwendung eines beliebigen LLM-Anbieters in Blackbox AI.
---

import config from "../../../../config.mjs"
export const console = config.console

Blackbox AI verwendet [AI SDK](https://ai-sdk.dev/) und [Models.dev](https://models.dev) zur Unterstützung von **75+ LLM-Anbietern** und unterstützt die Ausführung lokaler Modelle.

Um einen Anbieter hinzuzufügen, müssen Sie:

1. Fügen Sie die API-Schlüssel für den Anbieter mit dem Befehl `/connect` hinzu.
2. Konfigurieren Sie den Anbieter in Ihrer Blackbox AI-Konfiguration.
...
in `~/.local/share/blackbox ai/auth.json`.

---

### Konfiguration

Sie können die Anbieter über den Abschnitt `provider` in Ihrem Blackbox AI anpassen
config.
...
Blackbox AI Zen ist eine Liste von Modellen, die vom Blackbox AI-Team bereitgestellt wurden
Getestet und verifiziert, dass es gut mit Blackbox AI funktioniert. [Learn more](/docs/zen).

:::tip
Wenn Sie neu sind, empfehlen wir Ihnen, mit Blackbox AI Zen zu beginnen.
:::

1. Führen Sie den Befehl `/connect` in TUI aus, wählen Sie Blackbox AI aus und gehen Sie zu [blackbox.ai/auth](https://www.blackbox.ai/auth).
...
Es funktioniert wie jeder andere Anbieter in Blackbox AI und ist völlig optional.
...
So verwenden Sie Amazon Bedrock mit Blackbox AI:
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Legen Sie eine dieser Umgebungsvariablen fest, während Sie Blackbox AI ausführen:
   ...
   Für eine projektspezifische oder dauerhafte Konfiguration verwenden Sie `blackbox ai.json`:
...
:::info
Die Verwendung Ihres Claude Pro/Max-Abonnements in Blackbox AI wird von [Anthropic](https://anthropic.com) nicht offiziell unterstützt.
:::
...
2. Gehen Sie zu [Azure AI Foundry](https://ai.azure.com/) und stellen Sie ein Modell bereit.
   ...
   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit Blackbox AI ordnungsgemäß funktioniert.
   :::
...
5. Führen Sie den Befehl `/models` aus, um ein Modell auszuwählen.
   ...
   Sie können Modelle auch über Ihre Blackbox AI-Konfiguration hinzufügen.
...
2. Wählen Sie Ihre Authentifizierungsmethode:
   ...
   3. Name: `Blackbox AI`, Bereiche: `api`
...
um das Token im Blackbox AI-Authentifizierungsspeicher zu speichern.
:::

##### Selbstgehostetes GitLab

:::note[Compliance-Hinweis]
Blackbox AI verwendet ein kleines Modell für einige AI-Aufgaben wie das Generieren des Sitzungstitels.
Es ist standardmäßig für die Verwendung von gpt-5-nano konfiguriert und wird von Zen gehostet. Zum Sperren von Blackbox AI
Um nur Ihre eigene von GitLab gehostete Instanz zu verwenden, fügen Sie Folgendes zu Ihrer hinzu
`blackbox ai.json`-Datei. Es wird außerdem empfohlen, die Sitzungsfreigabe zu deaktivieren.
...
Weitere Dokumentation auf der [blackbox ai-gitlab-auth](https://www.npmjs.com/package/@gitlab/blackbox ai-gitlab-auth)-Homepage.

##### Konfiguration

Anpassen über `blackbox ai.json`:
...
So verwenden Sie Ihr GitHub Copilot-Abonnement mit Blackbox AI:
...
So verwenden Sie Google Vertex AI mit Blackbox AI:
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   ...
   Legen Sie sie fest, während Sie Blackbox AI ausführen.
...
Falls Sie eine Funktion oder ein Modell von Helicone sehen, das nicht automatisch über Blackbox AI konfiguriert wird, können Sie es jederzeit selbst konfigurieren.
...
Mit der [Sessions](https://docs.helicone.ai/features/sessions)-Funktion von Helicone können Sie zusammengehörige LLM-Anfragen gruppieren. Verwenden Sie das [blackbox ai-helicone-session](https://github.com/H2Shami/blackbox ai-helicone-session)-Plugin, um jede Blackbox AI-Konversation automatisch als Sitzung in Helicone zu protokollieren.
...
Das Plugin fügt `Helicone-Session-Id`- und `Helicone-Session-Name`-Header in Ihre Anfragen ein. Auf der Seite „Sitzungen“ von Helicone wird jede Blackbox AI-Konversation als separate Sitzung aufgeführt.
...
Sie können Blackbox AI für die Verwendung lokaler Modelle über das Dienstprogramm [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server konfigurieren
...
Sie können Blackbox AI über LM Studio für die Verwendung lokaler Modelle konfigurieren.
...
Sie können Blackbox AI über Ollama für die Verwendung lokaler Modelle konfigurieren.

:::tip
Ollama kann sich automatisch für Blackbox AI konfigurieren. Weitere Informationen finden Sie unter [Ollama integration docs](https://docs.ollama.com/integrations/blackbox ai).
:::
...
So verwenden Sie Ollama Cloud mit Blackbox AI:
...
3. Kopieren Sie den API-Schlüssel zur Verwendung in Blackbox AI.
...
6. **Wichtig**: Bevor Sie Cloud-Modelle in Blackbox AI verwenden, müssen Sie die Modellinformationen lokal abrufen:
...
Blackbox AI Zen ist eine Liste getesteter und verifizierter Modelle, die vom Blackbox AI-Team bereitgestellt werden. [Learn more](/docs/zen).

1. Melden Sie sich bei **<a href={console}>Blackbox AI Zen</a>** an und klicken Sie auf **Create API Key**.

2. Führen Sie den Befehl `/connect` aus und suchen Sie nach **Blackbox AI Zen**.

   ```txt
   /connect
   ```

3. Geben Sie Ihren Blackbox AI API-Schlüssel ein.
...
4. Viele OpenRouter-Modelle sind standardmäßig vorinstalliert. Führen Sie den Befehl `/models` aus, um das gewünschte Modell auszuwählen.
   ...
   Sie können auch zusätzliche Modelle über Ihre Blackbox AI-Konfiguration hinzufügen.
...
5. Sie können sie auch über Ihre Blackbox AI-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe eines Anbieters
...
So verwenden Sie [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) mit Blackbox AI:
...
Sie können Modelle auch über Ihre Blackbox AI-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe der Provider-Routing-Reihenfolge.
...
:::tip
Sie können jeden OpenAI-kompatiblen Anbieter mit Blackbox AI verwenden. Die meisten modernen AI-Anbieter bieten OpenAI-kompatible APIs an.
:::
...
4. Erstellen oder aktualisieren Sie Ihre `blackbox ai.json`-Datei in Ihrem Projektverzeichnis:
...
Mithilfe der `limit`-Felder kann Blackbox AI erkennen, wie viel Kontext Sie noch haben. Standardanbieter beziehen diese automatisch von models.dev.
...
1. **Überprüfen Sie die Authentifizierungseinrichtung**: Führen Sie `blackbox ai auth list` aus, um zu sehen, ob die Anmeldeinformationen vorhanden sind
...
2. Überprüfen Sie bei benutzerdefinierten Anbietern die Blackbox AI-Konfiguration und:
   - Stellen Sie sicher, dass der im Befehl `/connect` verwendete Anbieter ID mit dem ID in Ihrer Blackbox AI-Konfiguration übereinstimmt.
