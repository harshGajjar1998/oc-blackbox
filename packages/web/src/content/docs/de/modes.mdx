...
:::caution
Modi werden jetzt über die Option `agent` in der Blackbox AI-Konfiguration konfiguriert. Der
Die Option `mode` ist jetzt veraltet. [Learn more](/docs/agents).
:::

Mit den Modi in Blackbox AI können Sie das Verhalten, die Tools und die Eingabeaufforderungen für verschiedene Anwendungsfälle anpassen.

Es verfügt über zwei integrierte Modi: **Build** und **Plan**. Sie können anpassen
diese oder konfigurieren Sie Ihre eigenen über die Blackbox AI-Konfiguration.
...
Blackbox AI verfügt über zwei integrierte Modi.
...
- `edit` – Vorhandene Dateien können nicht geändert werden, mit Ausnahme der Dateien unter `.blackbox ai/plans/*.md`, um den Plan selbst detailliert darzustellen
...
Konfigurieren Sie Modi in Ihrer `blackbox ai.json`-Konfigurationsdatei:
...
Wenn keine Temperatur angegeben ist, verwendet Blackbox AI modellspezifische Standardwerte (normalerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle).
...
sowohl die globale Blackbox AI-Konfiguration als auch die projektspezifische Konfiguration.
...
Erstellen Sie Modusdateien in `.blackbox ai/modes/` für projektspezifische Modi oder `~/.config/blackbox ai/modes/` für globale Modi:
