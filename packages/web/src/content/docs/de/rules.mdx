---
title: Regeln
description: Lege eigene Anweisungen fuer blackbox ai fest.
---

Du kannst blackbox ai ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in blackbox ai den Befehl `/init` aus.
...
So kann blackbox ai sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht blackbox ai sie zu erweitern.
...
blackbox ai kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/blackbox ai/AGENTS.md` speichern.
Diese gelten in allen blackbox ai-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt Blackbox AI dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/blackbox ai/AGENTS.md` existiert)
...
Beim Start sucht blackbox ai in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/blackbox ai/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `blackbox ai.json` oder in `~/.config/blackbox ai/blackbox ai.json` angeben.
...
blackbox ai parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit blackbox ai.json

Empfohlen ist das `instructions`-Feld in der `blackbox ai.json`:
...
Du kannst blackbox ai explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `blackbox ai.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
