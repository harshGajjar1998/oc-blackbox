---
title: LSP Server
description: Blackbox AI lässt sich in Ihren LSP-Server integrieren.
---

Blackbox AI lässt sich in Ihr Language Server Protocol (LSP) integrieren, um die Interaktion von LLM mit Ihrer Codebasis zu unterstützen. Es verwendet Diagnosefunktionen, um dem LLM Feedback zu geben.

---

## Integriert

Blackbox AI verfügt über mehrere integrierte LSP-Server für gängige Sprachen:
...
:::note
Sie können LSP-Server-Downloads automatisch deaktivieren, indem Sie die Umgebungsvariable `BLACKBOX_AI_DISABLE_LSP_DOWNLOAD` auf `true` setzen.
:::
...
Wenn Blackbox AI eine Datei öffnet, geschieht Folgendes:
...
Sie können LSP-Server über den Abschnitt `lsp` in Ihrer Blackbox AI-Konfiguration anpassen.
...
```json title="blackbox ai.json" {5-7}
{
  "$schema": "https://www.blackbox.ai/config.json",
  "lsp": {
    "rust": {
...
```json title="blackbox ai.json" {5-9}
{
  "$schema": "https://www.blackbox.ai/config.json",
  "lsp": {
    "typescript": {
