...
Sie können externe Tools zu Blackbox AI hinzufügen, indem Sie das _Model Context Protocol_ oder MCP verwenden. Blackbox AI unterstützt sowohl lokale als auch Remote-Server.
...
Sie können den MCP-Server in Ihrem [Blackbox AI Config](https://www.blackbox.ai/docs/config/) unter `mcp` definieren. Fügen Sie jedem MCP einen eindeutigen Namen hinzu. Sie können bei der Eingabe von LLM namentlich auf diesen MCP verweisen.
...
Organisationen können über ihren `.well-known/blackbox ai`-Endpunkt Standard-MCP-Server bereitstellen. Diese Server sind möglicherweise standardmäßig deaktiviert, sodass Benutzer sich für den Server entscheiden können, die sie benötigen.
...
Blackbox AI übernimmt automatisch die OAuth-Authentifizierung für Remote-MCP-Server. Wenn ein Server eine Authentifizierung erfordert, wird Blackbox AI Folgendes tun:
...
Wenn der Server eine Authentifizierung erfordert, werden Sie von Blackbox AI beim ersten Versuch, ihn zu verwenden, zur Authentifizierung autorisiert. Wenn nicht, können Sie [manually trigger the flow](#authenticating) mit `blackbox ai mcp auth <server-name>` verwenden.
...
Der Befehl `mcp auth` öffnet Ihren Browser zur Autorisierung. Nach der Autorisierung speichert Blackbox AI die Token sicher in `~/.local/share/blackbox ai/mcp-auth.json`.
...
Ihre MCPs sind neben integrierten Tools auch als Tools in Blackbox AI verfügbar. Sie können sie also wie jedes andere Tool über die Blackbox AI-Konfiguration verwalten.
...
Dadurch wird ein Browserfenster geöffnet, um den OAuth-Ablauf abzuschließen und Blackbox AI mit Ihrem Sentry-Konto zu verbinden.
