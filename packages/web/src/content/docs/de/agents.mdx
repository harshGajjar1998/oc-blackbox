...
Es gibt zwei Arten von Agenten in Blackbox AI; Primäragenten und Subagenten.
...
Blackbox AI verfügt über zwei integrierte Primäragenten: **Build** und **Plan**. Auch
...
Konfigurieren Sie Agenten in Ihrer `blackbox ai.json`-Konfigurationsdatei:
...
- Pro Projekt: `.blackbox ai/agents/`
...
Wenn keine Temperatur angegeben ist, verwendet Blackbox AI modellspezifische Standardwerte; typischerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle.
...
Dieser Pfad ist relativ zum Speicherort der Konfigurationsdatei. Dies funktioniert sowohl für die globale Blackbox AI-Konfiguration als auch für die projektspezifische Konfiguration.
...
Die Modell-ID in Ihrer Blackbox AI-Konfiguration verwendet das Format `provider/model-id`. Wenn Sie beispielsweise [Blackbox AI Zen](/docs/zen) verwenden, würden Sie `blackbox ai/gpt-5.1-codex` für GPT 5.1 Codex verwenden.
...
:::tip
Führen Sie `blackbox ai models` aus, um eine Liste der verfügbaren Modelle anzubieten.
:::
...
:::tip
Haben Sie einen Agenten, den Sie gerne teilen möchten? [Reichen Sie einen PR ein](https://github.com/anomalyco/blackbox ai).
:::
