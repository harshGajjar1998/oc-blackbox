...
Wenn Blackbox AI Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `blackbox ai --log-level DEBUG`.

---

## Speicher

blackbox ai speichert Sitzungs- und App-Daten auf der Festplatte unter:
...
Blackbox AI Desktop startet im Hintergrund einen lokalen Blackbox AI-Server (`blackbox ai-cli`-Sidecar).
...
- Nur macOS: `Blackbox AI`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
Wenn du Probleme mit Blackbox AI hast:
