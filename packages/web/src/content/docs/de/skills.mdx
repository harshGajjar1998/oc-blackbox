...
Agent Skills erlauben Blackbox AI, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
Blackbox AI sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft Blackbox AI vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.blackbox ai/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/blackbox ai/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.blackbox ai/skills/git-release/SKILL.md` zum Beispiel so:
...
Blackbox AI listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `blackbox ai.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `blackbox ai.json`):
