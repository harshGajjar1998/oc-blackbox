...
description: Erstellen Sie Tools, die der LLM in Blackbox AI aufrufen kann.
---

Benutzerdefinierte Tools sind von Ihnen erstellte Funktionen, die der LLM während Gesprächen aufrufen kann. Sie arbeiten neben [built-in tools](/docs/tools) von Blackbox AI wie `read`, `write` und `bash`.
...
- Lokal, indem Sie sie im Verzeichnis `.blackbox ai/tools/` Ihres Projekts platzieren.
- Oder global, indem Sie sie in `~/.config/blackbox ai/tools/` platzieren.
