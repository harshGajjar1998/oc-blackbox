...
:::caution
Os modos agora são configurados através da opção `agent` na configuração do blackbox ai. A opção `mode` agora está obsoleta. [Saiba mais](/docs/agents).
:::

Os modos no blackbox ai permitem que você personalize o comportamento, as ferramentas e os prompts para diferentes casos de uso.

Ele vem com dois modos integrados: **build** e **plan**. Você pode personalizar esses ou configurar os seus próprios através da configuração do blackbox ai.
...
O blackbox ai vem com dois modos integrados.
...
- `edit` - Não pode modificar arquivos existentes, exceto para arquivos localizados em `.blackbox ai/plans/*.md` para detalhar o plano em si
...
Configure os modos no seu arquivo de configuração `blackbox ai.json`:
...
- Global: `~/.config/blackbox ai/modes/`
- Projeto: `.blackbox ai/modes/`

```markdown title="~/.config/blackbox ai/modes/review.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.1
tools:
...
Se nenhuma temperatura for especificada, o blackbox ai usa padrões específicos do modelo (geralmente 0 para a maioria dos modelos, 0.55 para modelos Qwen).
...
Este caminho é relativo a onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do blackbox ai quanto para a configuração específica do projeto.
...
```json title="blackbox ai.json" {4-14}
{
  "$schema": "https://www.blackbox.ai/config.json",
  "mode": {
    "docs": {
...
Crie arquivos de modo em `.blackbox ai/modes/` para modos específicos do projeto ou `~/.config/blackbox ai/modes/` para modos globais:

```markdown title=".blackbox ai/modes/debug.md"
---
temperature: 0.1
tools:
  bash: true
...
```markdown title="~/.config/blackbox ai/modes/refactor.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.2
tools:
