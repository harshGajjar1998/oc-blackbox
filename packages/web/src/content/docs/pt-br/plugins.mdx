---
title: Plugins
description: Escreva seus próprios plugins para estender o blackbox ai.
---

Plugins permitem que você estenda o blackbox ai conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do blackbox ai.
...
- `.blackbox ai/plugins/` - Plugins em nível de projeto
- `~/.config/blackbox ai/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/blackbox ai/node_modules/`.
...
1. Configuração global (`~/.config/blackbox ai/blackbox ai.json`)
2. Configuração do projeto (`blackbox ai.json`)
3. Diretório de plugins global (`~/.config/blackbox ai/plugins/`)
4. Diretório de plugins do projeto (`.blackbox ai/plugins/`)
...
O blackbox ai executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do blackbox ai para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o blackbox ai.
...
:::note
Se você estiver usando o aplicativo desktop blackbox ai, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o blackbox ai de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao blackbox ai:
...
O helper `tool` cria uma ferramenta personalizada que o blackbox ai pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o blackbox ai junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://www.blackbox.ai/docs/sdk) para detalhes.
