...
As ferramentas permitem que o LLM execute ações em sua base de código. O blackbox ai vem com um conjunto de ferramentas integradas, mas você pode estendê-lo com [ferramentas personalizadas](/docs/custom-tools) ou [servidores MCP](/docs/mcp-servers).
...
Aqui estão todas as ferramentas integradas disponíveis no blackbox ai.
...
:::note
Esta ferramenta está disponível apenas quando `BLACKBOX_AI_EXPERIMENTAL_LSP_TOOL=true` (ou `BLACKBOX_AI_EXPERIMENTAL=true`).
:::
...
:::note
Esta ferramenta está disponível apenas ao usar o provedor blackbox ai ou quando a variável de ambiente `BLACKBOX_AI_ENABLE_EXA` está definida como qualquer valor verdadeiro (por exemplo, `true` ou `1`).

Para habilitar ao iniciar o blackbox ai:
