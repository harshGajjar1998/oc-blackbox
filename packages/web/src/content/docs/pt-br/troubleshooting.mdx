...
Para depurar problemas com o blackbox ai, comece verificando os logs e os dados locais que ele armazena no disco.
...
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\blackbox ai\log`
...
Você pode definir o nível de log com a opção de linha de comando `--log-level` para obter informações de depuração mais detalhadas. Por exemplo, `blackbox ai --log-level DEBUG`.

---

## Armazenamento

O blackbox ai armazena dados de sessão e outros dados do aplicativo no disco em:

- **macOS/Linux**: `~/.local/share/blackbox ai/`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\blackbox ai`
...
O blackbox ai Desktop executa um servidor blackbox ai local (o sidecar `blackbox ai-cli`) em segundo plano. A maioria dos problemas é causada por um plugin com mau funcionamento, um cache corrompido ou uma configuração de servidor incorreta.
...
- Apenas macOS: menu `Blackbox AI` -> **Recarregar Webview** (ajuda se a interface estiver em branco/congelada).
...
- **macOS/Linux**: `~/.config/blackbox ai/blackbox ai.jsonc` (ou `~/.config/blackbox ai/blackbox ai.json`)
- **macOS/Linux** (instalações mais antigas): `~/.local/share/blackbox ai/blackbox ai.jsonc`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\blackbox ai\blackbox ai.jsonc`
...
O blackbox ai também pode carregar plugins locais do disco. Mova-os temporariamente para fora do caminho (ou renomeie a pasta) e reinicie o aplicativo de desktop:

- **Plugins globais**
  - **macOS/Linux**: `~/.config/blackbox ai/plugins/`
  - **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\blackbox ai\plugins`
...
Se desativar plugins não ajudar (ou se a instalação de um plugin estiver travada), limpe o cache para que o blackbox ai possa reconstruí-lo.

1. Saia completamente do blackbox ai Desktop.
2. Exclua o diretório de cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> cole `~/.cache/blackbox ai`
- **Linux**: exclua `~/.cache/blackbox ai` (ou execute `rm -rf ~/.cache/blackbox ai`)
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.cache\blackbox ai`

3. Reinicie o blackbox ai Desktop.

---

### Corrigindo problemas de conexão com o servidor

O blackbox ai Desktop pode iniciar seu próprio servidor local (padrão) ou conectar-se a uma URL de servidor que você configurou.
...
Se seu `blackbox ai.json(c)` contiver uma seção `server`, remova-a temporariamente e reinicie o aplicativo de desktop.

#### Verificando as variáveis de ambiente

Se você tiver `BLACKBOX_AI_PORT` definido em seu ambiente, o aplicativo de desktop tentará usar essa porta para o servidor local.

- Desfaça `BLACKBOX_AI_PORT` (ou escolha uma porta livre) e reinicie.
...
No Windows, o blackbox ai Desktop requer o **WebView2 Runtime** do Microsoft Edge. Se o aplicativo abrir em uma janela em branco ou não iniciar, instale/atualize o WebView2 e tente novamente.

---

### Windows: Problemas gerais de desempenho

Se você estiver enfrentando desempenho lento, problemas de acesso a arquivos ou problemas no terminal no Windows, tente usar [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). O WSL fornece um ambiente Linux que funciona de forma mais integrada com os recursos do blackbox ai.

---

### Notificações não aparecendo

O blackbox ai Desktop só mostra notificações do sistema quando:

- as notificações estão habilitadas para o blackbox ai nas configurações do seu sistema operacional, e
...
1. Saia do blackbox ai Desktop.
2. Encontre e exclua estes arquivos (eles estão no diretório de dados do aplicativo blackbox ai Desktop):

- `blackbox ai.settings.dat` (URL do servidor padrão do desktop)
- `blackbox ai.global.dat` e `blackbox ai.workspace.*.dat` (estado da interface como servidores/projetos recentes)
...
Se você estiver enfrentando problemas com o blackbox ai:
...
Para descobrir quais modelos você tem acesso, execute `blackbox ai models`
...
2. Se o problema persistir, tente limpar sua configuração armazenada:
   ...
   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.local\share\blackbox ai`
...
Se você encontrar erros de chamada de API, isso pode ser devido a pacotes de provedores desatualizados. O blackbox ai instala dinamicamente pacotes de provedores (OpenAI, Anthropic, Google, etc.) conforme necessário e os armazena em cache localmente.
...
1. Limpe o cache do pacote de provedores:
   ...
   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.cache\blackbox ai`

2. Reinicie o blackbox ai para reinstalar os pacotes de provedores mais recentes

Isso forçará o blackbox ai a baixar as versões mais recentes dos pacotes de provedores, o que muitas vezes resolve problemas de compatibilidade com parâmetros de modelo e alterações na API.
...
O blackbox ai detectará se você está usando Wayland e preferirá `wl-clipboard`, caso contrário, tentará encontrar ferramentas de área de transferência na ordem: `xclip` e `xsel`.
