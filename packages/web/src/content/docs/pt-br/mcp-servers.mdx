...
Você pode adicionar ferramentas externas ao blackbox ai usando o _Model Context Protocol_, ou MCP. O blackbox ai suporta servidores locais e remotos.
...
Você pode definir servidores MCP em sua [Configuração do blackbox ai](https://www.blackbox.ai/docs/config/) sob `mcp`. Adicione cada MCP com um nome único. Você pode se referir a esse MCP pelo nome ao solicitar ao LLM.
...
As organizações podem fornecer servidores MCP padrão através de seu endpoint `.well-known/blackbox ai`. Esses servidores podem estar desabilitados por padrão, permitindo que os usuários optem pelos que precisam.
...
O blackbox ai lida automaticamente com a autenticação OAuth para servidores MCP remotos. Quando um servidor requer autenticação, o blackbox ai irá:
...
Se o servidor requer autenticação, o blackbox ai solicitará que você se autentique quando tentar usá-lo pela primeira vez. Se não, você pode [iniciar manualmente o fluxo](#authenticating) com `blackbox ai mcp auth <server-name>`.
...
O comando `mcp auth` abrirá seu navegador para autorização. Após você autorizar, o blackbox ai armazenará os tokens de forma segura em `~/.local/share/blackbox ai/mcp-auth.json`.
...
Seus MCPs estão disponíveis como ferramentas no blackbox ai, juntamente com ferramentas integradas. Portanto, você pode gerenciá-los através da configuração do blackbox ai como qualquer outra ferramenta.
...
Isso abrirá uma janela do navegador para completar o fluxo OAuth e conectar o blackbox ai à sua conta do Sentry.
