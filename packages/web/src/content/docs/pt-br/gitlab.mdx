---
title: GitLab
description: Use o blackbox ai em problemas e solicitações de mesclagem do GitLab.
---

O blackbox ai se integra ao seu fluxo de trabalho do GitLab através do seu pipeline CI/CD do GitLab ou com o GitLab Duo.

Em ambos os casos, o blackbox ai será executado em seus runners do GitLab.

---

## GitLab CI

O blackbox ai funciona em um pipeline regular do GitLab. Você pode integrá-lo a um pipeline como um [componente CI](https://docs.gitlab.com/ee/ci/components/)

Aqui estamos usando um componente CI/CD criado pela comunidade para blackbox ai — [nagyv/gitlab-blackbox ai](https://gitlab.com/nagyv/gitlab-blackbox ai).

---

### Recursos

- **Use configuração personalizada por trabalho**: Configure o blackbox ai com um diretório de configuração personalizado, por exemplo `./config/#custom-directory` para habilitar ou desabilitar funcionalidades por invocação do blackbox ai.
- **Configuração mínima**: O componente CI configura o blackbox ai em segundo plano, você só precisa criar a configuração do blackbox ai e o prompt inicial.
...
1. Armazene seu JSON de autenticação do blackbox ai como variáveis de ambiente do tipo File em **Configurações** > **CI/CD** > **Variáveis**. Certifique-se de marcá-las como "Mascaradas e ocultas".
   ...
   include:
     - component: $CI_SERVER_FQDN/nagyv/gitlab-blackbox ai/blackbox ai@2
       inputs:
         ...
         auth_json: $BLACKBOX_AI_AUTH_JSON # O nome da variável para seu JSON de autenticação do Blackbox AI
         command: optional-custom-command
         message: "Seu prompt aqui"
   ```

Para mais entradas e casos de uso [consulte a documentação](https://gitlab.com/explore/catalog/nagyv/gitlab-blackbox ai) deste componente.

---

## GitLab Duo

O blackbox ai se integra ao seu fluxo de trabalho do GitLab.
Mencione `@blackbox ai` em um comentário, e o blackbox ai executará tarefas dentro do seu pipeline CI do GitLab.
...
- **Corrigir e implementar**: Peça ao blackbox ai para corrigir um problema ou implementar uma funcionalidade.
  Ele criará um novo branch e abrirá uma solicitação de mesclagem com as alterações.
- **Seguro**: O blackbox ai é executado em seus runners do GitLab.

---

### Configuração

O blackbox ai é executado em seu pipeline CI/CD do GitLab, aqui está o que você precisará para configurá-lo:
...
6.  Crie um arquivo de configuração de fluxo, aqui está um exemplo:
    ...
    commands:
      ...
      - echo $GITLAB_HOST
      - echo "Criando configuração de auth do Blackbox AI"
...
Aqui estão alguns exemplos de como você pode usar o blackbox ai no GitLab.

:::tip
Você pode configurar para usar uma frase de gatilho diferente de `@blackbox ai`.
:::
...
- **Explicar um problema**
  ...
  O blackbox ai lerá o problema e responderá com uma explicação clara.
...
- **Corrigir um problema**
  ...
  O blackbox ai criará um novo branch, implementará as alterações e abrirá uma solicitação de mesclagem com as alterações.
...
- **Revisar solicitações de mesclagem**
  ...
  O blackbox ai revisará a solicitação de mesclagem e fornecerá feedback.
