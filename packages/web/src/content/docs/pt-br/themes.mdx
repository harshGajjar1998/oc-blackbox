...
Com o blackbox ai, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o blackbox ai usa nosso próprio tema `blackbox ai`.
...
O blackbox ai vem com vários temas embutidos.
...
- Querem que o blackbox ai corresponda à aparência do seu terminal
...
O blackbox ai suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/blackbox ai/themes/*.json` ou `$XDG_CONFIG_HOME/blackbox ai/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.blackbox ai/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.blackbox ai/themes/*.json`
