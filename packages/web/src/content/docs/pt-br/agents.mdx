...
Existem dois tipos de agentes no blackbox ai; agentes primários e subagentes.
...
blackbox ai vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
blackbox ai vem com dois subagentes integrados, **General** e **Explore**. Vamos ver isso abaixo.

---

## Integrados

blackbox ai vem com dois agentes primários integrados e dois subagentes integrados.
...
Configure os agentes em seu arquivo de configuração `blackbox ai.json`:
...
- Por projeto: `.blackbox ai/agents/`
...
Se nenhuma temperatura for especificada, o blackbox ai usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do blackbox ai quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do blackbox ai usa o formato `provider/model-id`. Por exemplo, se você estiver usando [Blackbox AI Zen](/docs/zen), você usaria `blackbox ai/gpt-5.1-codex` para GPT 5.1 Codex.
...
:::tip
Execute `blackbox ai models` para ver uma lista dos modelos disponíveis.
:::
...
:::tip
Você tem um agente que gostaria de compartilhar? [Envie um PR](https://github.com/anomalyco/blackbox ai).
:::
