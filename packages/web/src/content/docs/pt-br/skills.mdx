...
As habilidades do agente permitem que o blackbox ai descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O blackbox ai pesquisa nesses locais:

- Configuração do projeto: `.blackbox ai/skills/<name>/SKILL.md`
- Configuração global: `~/.config/blackbox ai/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o blackbox ai sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.blackbox ai/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/blackbox ai/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.blackbox ai/skills/git-release/SKILL.md` assim:
...
O blackbox ai lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `blackbox ai.json`:
...
**Para agentes embutidos** (em `blackbox ai.json`):
