...
description: Crie ferramentas que o LLM pode chamar no blackbox ai.
---

Ferramentas personalizadas são funções que você cria e que o LLM pode chamar durante as conversas. Elas funcionam junto com as [ferramentas integradas](/docs/tools) do blackbox ai, como `read`, `write` e `bash`.
...
- Localmente, colocando-as no diretório `.blackbox ai/tools/` do seu projeto.
- Ou globalmente, colocando-as em `~/.config/blackbox ai/tools/`.
