---
title: Servidor
description: Interaja com o servidor blackbox ai via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `blackbox ai serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente blackbox ai pode usar.
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `blackbox ai.local` |
...
Defina `BLACKBOX_AI_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `blackbox ai`, ou defina `BLACKBOX_AI_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `blackbox ai serve` quanto ao `blackbox ai web`.

```bash
BLACKBOX_AI_SERVER_PASSWORD=your-password blackbox ai serve
```
...
Quando você executa `blackbox ai`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor blackbox ai para interagir com o blackbox ai programaticamente.
:::

Essa arquitetura permite que o blackbox ai suporte múltiplos clientes e permite que você interaja com o blackbox ai programaticamente.

Você pode executar `blackbox ai serve` para iniciar um servidor autônomo. Se você tiver o TUI do blackbox ai em execução, `blackbox ai serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do blackbox ai [IDE](/docs/ide).
...
O servidor blackbox ai expõe as seguintes APIs.
