...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. Blackbox AI posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w Blackbox AI.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `BLACKBOX_AI_EXPERIMENTAL_LSP_TOOL=true` (lub `BLACKBOX_AI_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy Blackbox AI lub gdy zmienna środowiskowa `BLACKBOX_AI_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu Blackbox AI:
