...
W Blackbox AI obowiązuje zasada dwóch typów agentów: Primary (Główni) i Subagents (Subagenci).
...
Blackbox AI zawiera dwa wbudowane agenty główne: **Build** i **Plan**. Przyjrzymy się im poniżej.
...
Blackbox AI ma dwóch wbudowanych subagentów: **General** i **Explore**. Przyjrzymy się im poniżej.

---

## Wbudowane

Blackbox AI ma dwa wbudowane agenty główne i dwa wbudowane subagenty.
...
Skonfiguruj agenta w pliku `blackbox ai.json`:
...
- Globalnie: `~/.config/blackbox ai/agents/`
- W projekcie: `.blackbox ai/agents/`
...
Jeśli nie zostanie skonfigurowana, Blackbox AI używa wartości domyślnej dla danego modelu; 0 dla modeli rozumowania (reasoning models), 0.55 dla modeli Qwen.
...
Ścieżka jest względna do miejsca, w którym znajduje się plik konfiguracyjny. Działa to zarówno w przypadku globalnej konfiguracji Blackbox AI, jak i konfiguracji specyficznej dla projektu.
...
Identyfikator modelu jest taki sam, jak w konfiguracji Blackbox AI w `provider/model-id`. Na przykład, [Blackbox AI Zen](/docs/zen), użyj `blackbox ai/gpt-5.1-codex` dla GPT 5.1 Codex.
...
:::tip
Uruchom `blackbox ai models`, aby zobaczyć dostępne modele.
:::
...
:::tip
Masz agenta, którym warto się podzielić? [Prześlij PR](https://github.com/anomalyco/blackbox ai).
:::
