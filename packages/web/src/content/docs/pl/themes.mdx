...
Dzięki blackbox ai możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie blackbox ai używa naszego własnego motywu `blackbox ai`.
...
blackbox ai ma kilka wbudowanych motywów.
...
- Want blackbox ai to match their terminal's appearance
...
blackbox ai obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/blackbox ai/themes/*.json` lub `$XDG_CONFIG_HOME/blackbox ai/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.blackbox ai/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.blackbox ai/themes/*.json`
