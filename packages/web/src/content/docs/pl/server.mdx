---
title: Serwer
description: Interakcja z serwerem blackbox ai za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `blackbox ai serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient blackbox ai.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `blackbox ai.local` |
...
Ustaw `BLACKBOX_AI_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `blackbox ai` lub ustaw `BLACKBOX_AI_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `blackbox ai serve`, jak i `blackbox ai web`.
...
Kiedy uruchomisz `blackbox ai`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera blackbox ai do programowej interakcji z blackbox ai.
:::

Ta architektura umożliwia obsługę wielu klientów przez blackbox ai i programową interakcję z kodem otwartym.

Możesz uruchomić `blackbox ai serve`, aby uruchomić samodzielny serwer. Jeśli masz
blackbox ai TUI działa, `blackbox ai serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki blackbox ai [IDE](/docs/ide).
...
Serwer blackbox ai udostępnia następujące interfejsy API.
