...
:::caution
Tryby są teraz konfigurowane za pomocą opcji `agent` w konfiguracji blackbox ai.
...
te lub skonfiguruj własne za pomocą konfiguracji blackbox ai.
...
blackbox ai ma dwa puste tryby.
...
- `edit` — Nie można zastosować naruszenia plików, z naruszeniem praw autorskich w `.blackbox ai/plans/*.md` w celu uszczegółowienia samego planu
...
Skonfiguruj tryb w pliku konfiguracyjnym `blackbox ai.json`:
...
- Globalnie: `~/.config/blackbox ai/modes/`
...
Jeśli nie ma wpływu na temperaturę, blackbox ai stosuje się narzędzia badawcze dla modelu (zwykle 0 dla największych modeli, 0,55 dla modeli Qwen).
...
zarówno globalna blackbox ai, jak i specjalna dla projektu.
...
Utwórz pliki trybów w `.blackbox ai/modes/` dla trybów zapisanych dla projektu lub `~/.config/blackbox ai/modes/` dla trybów globalnych:
