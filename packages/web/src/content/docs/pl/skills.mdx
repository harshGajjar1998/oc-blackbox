...
Umiejętności agenta pozwalają blackbox ai odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
blackbox ai przeszukuje te lokalizacje:

- Project config: `.blackbox ai/skills/<name>/SKILL.md`
- Global config: `~/.config/blackbox ai/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu blackbox ai przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.blackbox ai/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/blackbox ai/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.blackbox ai/skills/git-release/SKILL.md` w ten sposób:
...
blackbox ai wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `blackbox ai.json`:
...
**Dla agentów wbudowanych** (w `blackbox ai.json`):
