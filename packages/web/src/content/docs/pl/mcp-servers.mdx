...
Możesz dodać dodatkowe informacje do blackbox ai za pomocą _Model Context Protocol_, w skrócie MCP. blackbox ai obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [blackbox ai Config](https://www.blackbox.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/blackbox ai`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
blackbox ai automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, blackbox ai:
...
Jeśli serwer wymaga uwierzytelnienia, blackbox ai poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `blackbox ai mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji blackbox ai bezpieczne przechowa tokeny w `~/.local/share/blackbox ai/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w blackbox ai, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie blackbox ai, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć blackbox ai z kontem Sentry.
