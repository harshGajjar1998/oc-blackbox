...
Aby debugować problemy z blackbox ai, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\blackbox ai\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `blackbox ai --log-level DEBUG`.

---

## Przechowywanie danych

blackbox ai przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/blackbox ai/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\blackbox ai`
...
blackbox ai Desktop uruchamia w tle lokalny serwer blackbox ai (sidecar `blackbox ai-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `Blackbox AI` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/blackbox ai/blackbox ai.jsonc` (lub `~/.config/blackbox ai/blackbox ai.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/blackbox ai/blackbox ai.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\blackbox ai\blackbox ai.jsonc`
...
blackbox ai może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/blackbox ai/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\blackbox ai\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby blackbox ai mógł ją odbudować.

1. Całkowicie zamknij blackbox ai Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/blackbox ai`
- **Linux**: usuń `~/.cache/blackbox ai` (lub uruchom `rm -rf ~/.cache/blackbox ai`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\blackbox ai`

3. Uruchom ponownie blackbox ai Desktop.

---

### Napraw problemy z połączeniem z serwerem

blackbox ai Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `blackbox ai.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `BLACKBOX_AI_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `BLACKBOX_AI_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
W systemie Windows blackbox ai Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami blackbox ai.

---

### Brak powiadomień

blackbox ai Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla blackbox ai w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie blackbox ai Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji blackbox ai Desktop):

- `blackbox ai.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `blackbox ai.global.dat` i `blackbox ai.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
Jeśli masz problemy z blackbox ai:
...
3. Upewnij się, że masz najnowszą wersję z `blackbox ai upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `blackbox ai models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\blackbox ai`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. blackbox ai dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\blackbox ai`

2. Uruchom ponownie kod blackbox ai, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to blackbox ai do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
blackbox ai wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
