...
Narzędzia stosowane do funkcji, z których LLM może korzystać podczas rozmów. Współpracują z [wbudowanymi narzędziami] (./tools) blackbox ai, wtyczka jak `read`, `write` i `bash`.
...
- Lokalnie, umieszczając je w katalogu `.blackbox ai/tools/` swojego projektu.
- Lub globalnie, umieszczając je w `~/.config/blackbox ai/tools/`.
