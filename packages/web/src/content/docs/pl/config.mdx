---
title: Konfiguracja
description: Ustawienia Blackbox AI w pliku JSON.
---

Możesz dostosować Blackbox AI za pomocą pliku konfiguracyjnego JSON.

---

## Format

Blackbox AI obsługuje formaty **JSON** i **JSONC** (JSON z komentarzami).
...
Na przykład, jeśli konfiguracja globalna ustawi `theme: "blackbox ai"` i `autoupdate: true`, a konfiguracja projektu ustawi `model: "anthropic/claude-sonnet-4-5"`, ostateczna konfiguracja będzie zawierać wszystkie trzy ustawienia.
...
1. **Konfiguracja zdalna** (z `.well-known/blackbox ai`) – ustawienia wymuszane przez organizację
2. **Konfiguracja globalna** (`~/.config/blackbox ai/blackbox ai.json`) – preferencje użytkownika
3. **Konfiguracja niestandardowa** (zmienna `BLACKBOX_AI_CONFIG`) — jawne nadpisanie
4. **Konfiguracja projektu** (`blackbox ai.json` w projekcie) - ustawienia specyficzne dla projektu
5. **Katalogi `.blackbox ai`** - agenci, polecenia, umiejętności
6. **Konfiguracja wbudowana** (zmienna `BLACKBOX_AI_CONFIG_CONTENT`) — nadpisanie środowiska uruchomieniowego
...
:::note
W katalogach `.blackbox ai` i `~/.config/blackbox ai` używane są **nazwy w liczbie mnogiej** dla podkatalogów: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Nazwy w liczbie pojedynczej (np. `agent/`) są również obsługiwane dla kompatybilności wstecznej.
:::
...
Organizacje mogą publikować konfigurację pod adresem `.well-known/blackbox ai`. Jest ona pobierana automatycznie po uwierzytelnieniu u dostawcy, który ją obsługuje.
...
Umieść swoją globalną konfigurację Blackbox AI w `~/.config/blackbox ai/blackbox ai.json`. Użyj jej do ustawień ogólnych dla użytkownika, takich jak motywy, domyślny dostawca lub skróty klawiszowe.
...
Dodaj `blackbox ai.json` w katalogu głównym projektu. Konfiguracja projektu ma najwyższy priorytet wśród plików konfiguracyjnych — nadpisuje konfiguracje globalne i zdalne.

:::tip
Dodaj ten plik do kontroli wersji, aby udostępniać konfigurację zespołowi.
:::

Kiedy Blackbox AI się uruchamia, szuka pliku konfiguracyjnego w katalogu głównym repozytorium Git.
...
Możesz załadować niestandardowy plik konfiguracyjny, ustawiając zmienną środowiskową `BLACKBOX_AI_CONFIG`.
...
Możesz ustawić niestandardowy katalog konfiguracyjny za pomocą zmiennej środowiskowej `BLACKBOX_AI_CONFIG_DIR`. Katalog ten będzie przeszukiwany pod kątem agentów, poleceń, trybów i wtyczek, tak jak standardowy katalog `.blackbox ai` i powinien zachować tę samą strukturę.
...
Katalog niestandardowy jest ładowany po globalnej konfiguracji i katalogach `.blackbox ai`, więc **może być łączony** z ich zawartością.

---

## Schemat

Plik konfiguracyjny ma schemat JSON dostępny pod adresem [**`blackbox.ai/config.json`**](https://www.blackbox.ai/config.json).
...
Możesz skonfigurować serwer dla `blackbox ai serve` i `blackbox ai web` za pomocą opcji `server`.
...
- `mdns` - Włącz wykrywanie usług mDNS. Dzięki temu inne urządzenia w sieci mogą łatwo znaleźć Twój serwer Blackbox AI.
- `mdnsDomain` - Niestandardowa nazwa domeny dla usług mDNS. Domyślnie `blackbox ai.local`. Przydatne do uruchamiania wielu instancji w tej samej sieci.
...
Opcja `small_model` konfiguruje oddzielny model do lżejszych zadań, takich jak generowanie tytułów. Domyślnie Blackbox AI wybiera tańszy model od tego samego dostawcy, jeśli jest dostępny, w przeciwnym razie powraca do modelu głównego.
...
Skonfiguruj motyw interfejsu Blackbox AI za pomocą opcji `theme`.
...
Możesz także definiować agentów przy użyciu plików markdown w `~/.config/blackbox ai/agents/` lub `.blackbox ai/agents/`. [Dowiedz się więcej o agentach](/docs/agents).
...
Agent domyślny musi być agentem typu `primary` (nie `subagent`). Może to być agent wbudowany, taki jak `"build"` lub `"plan"`, albo zdefiniowany przez Ciebie [agent niestandardowy](/docs/agents). Jeśli agent nie istnieje lub jest subagentem, Blackbox AI powróci do `"build"` z ostrzeżeniem.

Dotyczy to wszystkich interfejsów: TUI, CLI (`blackbox ai run`), aplikacji desktopowej i GitHub Actions.
...
Możesz także definiować polecenia przy użyciu plików Markdown w `~/.config/blackbox ai/commands/` lub `.blackbox ai/commands/`. [Dowiedz się więcej o poleceniach](/docs/commands).
...
Blackbox AI może automatycznie pobierać nowe wersje. Możesz to kontrolować za pomocą opcji `autoupdate`.
...
Działa to tylko wtedy, gdy Blackbox AI nie został zainstalowany przez menedżera pakietów, takiego jak Homebrew.
...
Domyślnie Blackbox AI **zezwala na wszystkie działania** bez konieczności zatwierdzania. Możesz to zmienić za pomocą opcji `permission`.
...
[Wtyczki](/docs/plugins) rozszerzają Blackbox AI o niestandardowe narzędzia, hooki i integracje.

Umieść pliki wtyczek w `.blackbox ai/plugins/` lub `~/.config/blackbox ai/plugins/`. Możesz także załadować wtyczkę z npm za pomocą opcji `plugin`.
...
Jest to przydatne, gdy chcesz ograniczyć użycie Blackbox AI tylko do określonych dostawców.
