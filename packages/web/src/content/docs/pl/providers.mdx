---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w blackbox ai.
---

import config from "../../../../config.mjs"
export const console = config.console

blackbox ai używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji blackbox ai.
...
w `~/.local/share/blackbox ai/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim blackbox ai
config.
...
Blackbox AI Zen to lista modeli dostarczonych przez zespół blackbox ai, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z blackbox ai. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od Blackbox AI Zen.
:::
...
Działa jak każdy inny dostawca w blackbox ai i jest całkowicie opcjonalny w użyciu.
...
Aby używać Amazon Bedrock z blackbox ai:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania blackbox ai:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `blackbox ai.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w blackbox ai nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod blackbox ai działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji blackbox ai.
...
blackbox ai używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować blackbox ai
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`blackbox ai.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [blackbox ai-gitlab-auth](https://www.npmjs.com/package/@gitlab/blackbox ai-gitlab-auth).

##### Konfiguracja

Customize through `blackbox ai.json`:
...
Aby korzystać z subskrypcji GitHub Copilot z kodem blackbox ai:
...
Aby używać Google Vertex AI z blackbox ai:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Set them while running blackbox ai.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą blackbox ai, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [blackbox ai-helicone-session](https://github.com/H2Shami/blackbox ai-helicone-session), aby automatycznie rejestrować każdą konwersację blackbox ai jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja blackbox ai będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować blackbox ai tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować blackbox ai tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować blackbox ai tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem blackbox ai. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/blackbox ai).
:::
...
Aby korzystać z Ollama Cloud z blackbox ai:
...
3. Skopiuj klucz API do użycia w blackbox ai.
...
6. **Ważne**: Przed użyciem modeli chmurowych w blackbox ai musisz pobrać informacje o modelu lokalnie:
...
Blackbox AI Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół blackbox ai. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>Blackbox AI Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **Blackbox AI Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API blackbox ai.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji blackbox ai.
...
5. Możesz je także dostosować za pomocą konfiguracji blackbox ai. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z blackbox ai:
...
Możesz także dostosować modele za pomocą konfiguracji blackbox ai. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z blackbox ai. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `blackbox ai.json` w katalogu projektu:
...
Pola `limit` pozwalają blackbox ai zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `blackbox ai auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację blackbox ai i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji blackbox ai.
