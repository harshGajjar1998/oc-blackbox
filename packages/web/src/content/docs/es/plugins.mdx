---
title: Complementos
description: Escriba sus propios complementos para extender Blackbox AI.
---

Los complementos le permiten extender Blackbox AI al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de Blackbox AI.
...
- `.blackbox ai/plugins/` - Complementos a nivel de proyecto
- `~/.config/blackbox ai/plugins/` - Complementos globales
...
```json title="blackbox ai.json"
{
  ...
  "plugin": ["blackbox ai-helicone-session", "blackbox ai-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/blackbox ai/node_modules/`.
...
1. Configuración global (`~/.config/blackbox ai/blackbox ai.json`)
2. Configuración del proyecto (`blackbox ai.json`)
3. Directorio global de complementos (`~/.config/blackbox ai/plugins/`)
4. Directorio de complementos del proyecto (`.blackbox ai/plugins/`)
...
```json title=".blackbox ai/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
Blackbox AI ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".blackbox ai/plugins/my-plugin.ts"
...
```js title=".blackbox ai/plugins/example.js"
...
- `client`: Un cliente SDK blackbox ai para interactuar con la IA.
...
import type { Plugin } from "@blackbox ai-ai/plugin"
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar blackbox ai.
...
```js title=".blackbox ai/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "blackbox ai"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio Blackbox AI, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que blackbox ai lea archivos `.env`:

```javascript title=".blackbox ai/plugins/env-protection.js"
...
```javascript title=".blackbox ai/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a blackbox ai:

```ts title=".blackbox ai/plugins/custom-tools.ts"
import { type Plugin, tool } from "@blackbox ai-ai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que blackbox ai puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para blackbox ai junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://www.blackbox.ai/docs/sdk) para obtener más detalles.
...
```ts title=".blackbox ai/plugins/compaction.ts"
import type { Plugin } from "@blackbox ai-ai/plugin"
...
```ts title=".blackbox ai/plugins/custom-compaction.ts"
import type { Plugin } from "@blackbox ai-ai/plugin"
