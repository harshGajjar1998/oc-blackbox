---
title: Servidor
description: Interactuar con el servidor blackbox ai a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `blackbox ai serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente blackbox ai puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `blackbox ai.local` |
...
Configure `BLACKBOX_AI_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `blackbox ai`, o configure `BLACKBOX_AI_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `blackbox ai serve` como a `blackbox ai web`.
...
Cuando ejecuta `blackbox ai`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor blackbox ai para interactuar con blackbox ai mediante programación.
:::

Esta arquitectura permite que blackbox ai admita múltiples clientes y le permite interactuar con blackbox ai mediante programación.

Puede ejecutar `blackbox ai serve` para iniciar un servidor independiente. Si tienes el
blackbox ai TUI ejecutándose, `blackbox ai serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos Blackbox AI [IDE](/docs/ide).
...
El servidor blackbox ai expone las siguientes API.
