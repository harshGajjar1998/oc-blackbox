...
Las herramientas permiten que LLM realice acciones en su código base. Blackbox AI viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en Blackbox AI.
...
:::note
Esta herramienta solo está disponible cuando `BLACKBOX_AI_EXPERIMENTAL_LSP_TOOL=true` (o `BLACKBOX_AI_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor Blackbox AI o cuando la variable de entorno `BLACKBOX_AI_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar Blackbox AI:

```bash
BLACKBOX_AI_ENABLE_EXA=1 blackbox ai
```
