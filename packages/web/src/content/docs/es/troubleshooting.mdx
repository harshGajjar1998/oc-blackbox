...
Para depurar problemas con Blackbox AI, comience verificando los registros y los datos locales que almacena en el disco.
...
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\blackbox ai\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `blackbox ai --log-level DEBUG`.

---

## Almacenamiento

blackbox ai almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/blackbox ai/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\blackbox ai`
...
Blackbox AI Desktop ejecuta un servidor Blackbox AI local (el sidecar `blackbox ai-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `Blackbox AI` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/blackbox ai/blackbox ai.jsonc` (o `~/.config/blackbox ai/blackbox ai.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/blackbox ai/blackbox ai.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\blackbox ai\blackbox ai.jsonc`
...
Blackbox AI también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/blackbox ai/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\blackbox ai\plugins`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que Blackbox AI pueda reconstruirlo.

1. Salga de Blackbox AI Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/blackbox ai`
- **Linux**: eliminar `~/.cache/blackbox ai` (o ejecutar `rm -rf ~/.cache/blackbox ai`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\blackbox ai`

3. Reinicie el escritorio Blackbox AI.

---

### Solucionar problemas de conexión del servidor

Blackbox AI Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `blackbox ai.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.

#### Verificar variables de entorno

Si tiene `BLACKBOX_AI_PORT` configurado en su entorno, la aplicación de escritorio intentará usar ese puerto para el servidor local.

- Desarme `BLACKBOX_AI_PORT` (o elija un puerto libre) y reinicie.
...
En Windows, el escritorio Blackbox AI requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de Blackbox AI.

---

### Notificaciones que no se muestran

Blackbox AI Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para Blackbox AI en la configuración de su sistema operativo, y
...
1. Salga del escritorio Blackbox AI.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio Blackbox AI):

- `blackbox ai.settings.dat` (URL del servidor predeterminado de escritorio)
- `blackbox ai.global.dat` y `blackbox ai.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
Si tiene problemas con Blackbox AI:
...
3. Asegúrese de tener la última versión con `blackbox ai upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `blackbox ai models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\blackbox ai`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. blackbox ai instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\blackbox ai`

2. Reinicie blackbox ai para reinstalar los paquetes del proveedor más recientes.

Esto obligará a blackbox ai a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
blackbox ai detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
