...
Puede agregar herramientas externas a Blackbox AI utilizando el _Model Context Protocol_ o MCP. Blackbox AI admite servidores locales y remotos.
...
Puede definir servidores MCP en su [Blackbox AI Config](https://www.blackbox.ai/docs/config/) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
Las organizaciones pueden proporcionar servidores MCP predeterminados a través de su punto final `.well-known/blackbox ai`. Estos servidores pueden estar deshabilitados de forma predeterminada, lo que permite a los usuarios optar por los que necesitan.
...
Blackbox AI maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, Blackbox AI:
...
Si el servidor requiere autenticación, Blackbox AI le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `blackbox ai mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, Blackbox AI almacenará los tokens de forma segura en `~/.local/share/blackbox ai/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en Blackbox AI, junto con herramientas integradas. Para que puedas administrarlos a través de la configuración Blackbox AI como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar Blackbox AI a su cuenta Sentry.
