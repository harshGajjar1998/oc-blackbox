...
You can add external tools to Blackbox AI using the _Model Context Protocol_, or MCP. Blackbox AI supports both local and remote servers.
...
You can define MCP servers in your [Blackbox AI Config](https://www.blackbox.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/blackbox ai` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
Blackbox AI automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Blackbox AI will:
...
If the server requires authentication, Blackbox AI will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `blackbox ai mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, Blackbox AI will store the tokens securely in `~/.local/share/blackbox ai/mcp-auth.json`.
...
Your MCPs are available as tools in Blackbox AI, alongside built-in tools. So you can manage them through the Blackbox AI config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect Blackbox AI to your Sentry account.
