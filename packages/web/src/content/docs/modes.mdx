...
:::caution
Modes are now configured through the `agent` option in the blackbox ai config. The
`mode` option is now deprecated. [Learn more](/docs/agents).
:::

Modes in blackbox ai allow you to customize the behavior, tools, and prompts for different use cases.

It comes with two built-in modes: **build** and **plan**. You can customize
these or configure your own through the blackbox ai config.
...
blackbox ai comes with two built-in modes.
...
- `edit` - Cannot modify existing files, except for files located at `.blackbox ai/plans/*.md` to detail the plan itself
...
Configure modes in your `blackbox ai.json` config file:
...
both the global blackbox ai config and the project specific config.
...
Create mode files in `.blackbox ai/modes/` for project-specific modes or `~/.config/blackbox ai/modes/` for global modes:
