---
title: Providers
description: Using any LLM provider in Blackbox AI.
---

import config from "../../../config.mjs"
export const console = config.console

Blackbox AI uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your Blackbox AI config.
...
You can customize the providers through the `provider` section in your Blackbox AI
config.
...
Blackbox AI Zen is a list of models provided by the Blackbox AI team that have been
tested and verified to work well with Blackbox AI. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with Blackbox AI Zen.
:::

1. Run the `/connect` command in the TUI, select `Blackbox AI Zen`, and head to [blackbox.ai/auth](https://www.blackbox.ai/zen).
...
It works like any other provider in Blackbox AI and is completely optional to use.

---

## Blackbox AI Go

Blackbox AI Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the Blackbox AI team that have been
tested and verified to work well with Blackbox AI.

1. Run the `/connect` command in the TUI, select `Blackbox AI Go`, and head to [blackbox.ai/auth](https://www.blackbox.ai/zen).
...
To use Amazon Bedrock with Blackbox AI:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running blackbox ai:
   ...
   ***
   ...
   For project-specific or persistent configuration, use `blackbox ai.json`:
...
:::info
Using your Claude Pro/Max subscription in Blackbox AI is not officially supported by [Anthropic](https://anthropic.com).
:::
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for blackbox ai to work properly.
   :::
...
5. Run the `/models` command to select a model.
   ...
   You can also add models through your blackbox ai config.
...
Blackbox AI uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock Blackbox AI
to only use your own GitLab-hosted instance, add the following to your
`blackbox ai.json` file. It is also recommended to disable session sharing.
...
More documentation on [blackbox ai-gitlab-auth](https://www.npmjs.com/package/@gitlab/blackbox ai-gitlab-auth) homepage.
...
To use your GitHub Copilot subscription with blackbox ai:
...
To use Google Vertex AI with Blackbox AI:
...
In the event you see a feature or model from Helicone that isn't configured automatically through blackbox ai, you can always configure it yourself.
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [blackbox ai-helicone-session](https://github.com/H2Shami/blackbox ai-helicone-session) plugin to automatically log each Blackbox AI conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each Blackbox AI conversation listed as a separate session.
...
You can configure blackbox ai to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility
...
You can configure blackbox ai to use local models through LM Studio.
...
You can configure blackbox ai to use local models through Ollama.

:::tip
Ollama can automatically configure itself for Blackbox AI. See the [Ollama integration docs](https://docs.ollama.com/integrations/blackbox ai) for details.
:::
...
To use Ollama Cloud with Blackbox AI:
...
3. Copy the API key for use in Blackbox AI.
...
6. **Important**: Before using cloud models in Blackbox AI, you must pull the model information locally:
...
Blackbox AI Zen is a list of tested and verified models provided by the Blackbox AI team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>Blackbox AI Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **Blackbox AI Zen**.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your blackbox ai config.
...
5. You can also customize them through your blackbox ai config. Here's an example of specifying a provider
...
To use [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) with Blackbox AI:
...
You can also customize models through your blackbox ai config. Here's an example of specifying provider routing order.
...
:::tip
You can use any OpenAI-compatible provider with blackbox ai. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
4. Create or update your `blackbox ai.json` file in your project directory:
...
The `limit` fields allow Blackbox AI to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `blackbox ai auth list` to see if the credentials
...
2. For custom providers, check the blackbox ai config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your blackbox ai config.
