---
title: Tjener
description: Samhandle med Blackbox AI-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `blackbox ai serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en Blackbox AI-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `blackbox ai.local` |
...
Sett `BLACKBOX_AI_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `blackbox ai` som standard, eller sett `BLACKBOX_AI_SERVER_USERNAME` for å overstyre det. Dette gjelder både `blackbox ai serve` og `blackbox ai web`.
...
Når du kjører `blackbox ai` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk Blackbox AI-serveren til å samhandle med Blackbox AI programmatisk.
:::

Denne arkitekturen lar Blackbox AI støtte flere klienter og lar deg samhandle med Blackbox AI programmatisk.

Du kan kjøre `blackbox ai serve` for å starte en frittstående server. Hvis du har
Blackbox AI TUI kjører, vil `blackbox ai serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av Blackbox AI [IDE](/docs/ide) plugins.
...
blackbox ai-serveren viser følgende APIer.
