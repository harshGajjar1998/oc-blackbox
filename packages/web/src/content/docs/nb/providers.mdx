---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i Blackbox AI.
---

import config from "../../../../config.mjs"
export const console = config.console

Blackbox AI bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i Blackbox AI-konfigurasjonen.
...
i `~/.local/share/blackbox ai/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i Blackbox AI
konfig.
...
Blackbox AI Zen er en liste over modeller levert av Blackbox AI-teamet som har vært
testet og verifisert for å fungere godt med Blackbox AI. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med Blackbox AI Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg blackbox ai og gå til [blackbox.ai/auth](https://www.blackbox.ai/auth).
...
Det fungerer som alle andre leverandører i Blackbox AI og er helt valgfritt å bruke.
...
Slik bruker du Amazon Bedrock med Blackbox AI:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører Blackbox AI:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `blackbox ai.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i blackbox ai støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at Blackbox AI skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom Blackbox AI-konfigurasjonen.
...
for å lagre token i Blackbox AI auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
Blackbox AI bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse Blackbox AI
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`blackbox ai.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [blackbox ai-gitlab-auth](https://www.npmjs.com/package/@gitlab/blackbox ai-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `blackbox ai.json`:
...
Slik bruker du GitHub Copilot-abonnementet med Blackbox AI:
...
Slik bruker du Google Vertex AI med Blackbox AI:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører Blackbox AI.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom Blackbox AI, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [blackbox ai-helicone-session](https://github.com/H2Shami/blackbox ai-helicone-session) plugin for automatisk å logge hver Blackbox AI-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver Blackbox AI-samtale oppført som en separat økt.
...
Du kan konfigurere Blackbox AI for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere Blackbox AI for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere Blackbox AI for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for Blackbox AI. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/blackbox ai) for detaljer.
:::
...
Slik bruker du Ollama Cloud med Blackbox AI:
...
3. Kopier API-nøkkelen for bruk i Blackbox AI.
...
6. **Viktig**: Før du bruker skymodeller i Blackbox AI, må du hente modellinformasjonen lokalt:
...
Blackbox AI Zen er en liste over testede og verifiserte modeller levert av Blackbox AI-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>Blackbox AI Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **Blackbox AI Zen**.

   ```txt
   /connect
   ```

3. Skriv inn Blackbox AI API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom Blackbox AI-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom Blackbox AI-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Blackbox AI:
...
Du kan også tilpasse modeller gjennom Blackbox AI-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med Blackbox AI. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `blackbox ai.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar Blackbox AI forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `blackbox ai auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk Blackbox AI-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i Blackbox AI-konfigurasjonen.
