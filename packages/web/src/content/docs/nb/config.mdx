---
title: Konfigurasjon
description: Ved å bruke blackbox ai JSON-konfigurasjonen.
---

Du kan konfigurere Blackbox AI ved å bruke en JSON konfigurasjonsfil.

---

## Format

Blackbox AI støtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
For eksempel, hvis din globale konfigurasjon setter `theme: "blackbox ai"` og `autoupdate: true`, og prosjektkonfigurasjonen setter `model: "anthropic/claude-sonnet-4-5"`, vil den endelige konfigurasjonen inkludere alle tre innstillingene.
...
1. **Ekstern konfigurasjon** (fra `.well-known/blackbox ai`) - organisasjonsstandarder
2. **Global konfigurasjon** (`~/.config/blackbox ai/blackbox ai.json`) - brukerinnstillinger
3. **Egendefinert konfigurasjon** (`BLACKBOX_AI_CONFIG` env var) - egendefinerte overstyringer
4. **Prosjektkonfigurasjon** (`blackbox ai.json` i prosjekt) - prosjektspesifikke innstillinger
5. **`.blackbox ai` kataloger** - agenter, kommandoer, plugins
6. **Inline config** (`BLACKBOX_AI_CONFIG_CONTENT` env var) - kjøretidsoverstyringer
...
:::note
`.blackbox ai` og `~/.config/blackbox ai` katalogene bruker **flertallsnavn** på underkataloger: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkeltnavn (f.eks. `agent/`) støttes også for bakoverkompatibilitet.
:::
...
Organisasjoner kan tilby standardkonfigurasjon via `.well-known/blackbox ai`-endepunktet. Dette hentes automatisk når du autentiserer med en leverandør som støtter det.
...
Plasser din globale Blackbox AI-konfigurasjon i `~/.config/blackbox ai/blackbox ai.json`. Bruk global konfigurasjon for brukerspesifikke preferanser som temaer, leverandører eller nøkkelbindinger.
...
Legg til `blackbox ai.json` i prosjektroten din. Prosjektkonfigurasjon har den høyeste prioritet blant standard config-filer - den overstyrer både globale og eksterne konfigurasjoner.

:::tip
Plasser prosjektspesifikk konfigurasjon i roten til prosjektet ditt.
:::

Når Blackbox AI starter opp, ser den etter en konfigurasjonsfil i gjeldende katalog eller går opp til nærmeste Git-katalog.
...
Spesifiser en tilpasset konfigurasjonsfilbane ved å bruke miljøvariabelen `BLACKBOX_AI_CONFIG`.
...
Spesifiser en tilpasset konfigurasjonskatalog ved å bruke `BLACKBOX_AI_CONFIG_DIR`
miljøvariabel. Denne katalogen vil bli søkt etter agenter, kommandoer,
moduser og plugins akkurat som standard `.blackbox ai` katalog, og bør
...
Den egendefinerte katalogen lastes inn etter den globale konfigurasjonen og `.blackbox ai`-katalogene, slik at den **kan overstyre** innstillingene deres.

---

## Skjema

Konfigurasjonsfilen har et skjema som er definert i [**`blackbox.ai/config.json`**](https://www.blackbox.ai/config.json).
...
Du kan konfigurere serverinnstillinger for kommandoene `blackbox ai serve` og `blackbox ai web` gjennom alternativet `server`.
...
- `mdns` - Aktiver mDNS-tjenesteoppdagelse. Dette lar andre enheter på nettverket oppdage Blackbox AI-serveren din.
- `mdnsDomain` - Egendefinert domenenavn for mDNS-tjeneste. Standard er `blackbox ai.local`. Nyttig for å kjøre flere forekomster på samme nettverk.
...
Du kan konfigurere leverandørene og modellene du vil bruke i Blackbox AI-konfigurasjonen gjennom alternativene `provider`, `model` og `small_model`.
...
Alternativet `small_model` konfigurerer en egen modell for lette oppgaver som tittelgenerering. Som standard prøver Blackbox AI å bruke en rimeligere modell hvis en er tilgjengelig fra leverandøren din, ellers faller den tilbake til hovedmodellen din.
...
Du kan konfigurere temaet du vil bruke i Blackbox AI-konfigurasjonen gjennom alternativet `theme`.
...
Du kan også definere agenter ved å bruke markdown-filer i `~/.config/blackbox ai/agents/` eller `.blackbox ai/agents/`. [Les mer her](/docs/agents).
...
Standardagenten må være en primæragent (ikke en underagent). Dette kan være en innebygd agent som `"build"` eller `"plan"`, eller en [egendefinert agent](/docs/agents) du har definert. Hvis den angitte agenten ikke eksisterer eller er en subagent, vil Blackbox AI falle tilbake til `"build"` med en advarsel.

Denne innstillingen gjelder for alle grensesnitt: TUI, CLI (`blackbox ai run`), skrivebordsapp og GitHub Action.
...
Du kan også definere kommandoer ved å bruke markdown-filer i `~/.config/blackbox ai/commands/` eller `.blackbox ai/commands/`. [Les mer her](/docs/commands).
...
Blackbox AI vil automatisk laste ned eventuelle nye oppdateringer når den starter opp. Du kan deaktivere dette med alternativet `autoupdate`.
...
Som standard **tillater Blackbox AI alle operasjoner** uten å kreve eksplisitt godkjenning. Du kan endre dette ved å bruke alternativet `permission`.
...
[Plugins](/docs/plugins) utvider Blackbox AI med tilpassede verktøy, kroker og integrasjoner.

Plasser plugin-filer i `.blackbox ai/plugins/` eller `~/.config/blackbox ai/plugins/`. Du kan også laste inn plugins fra npm gjennom alternativet `plugin`.
...
Dette er nyttig når du vil begrense Blackbox AI til kun å bruke spesifikke leverandører i stedet for å deaktivere dem én etter én.
