...
Du kan legge til eksterne verktøy i Blackbox AI ved å bruke _Model Context Protocol_ eller MCP. Blackbox AI støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [Blackbox AI Config](https://www.blackbox.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="blackbox ai.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/blackbox ai`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
```jsonc title="blackbox ai.jsonc" {15}
{
  "$schema": "https://www.blackbox.ai/config.json",
  "mcp": {
    "my-local-mcp-server": {
...
Blackbox AI håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil Blackbox AI:
...
Hvis serveren krever autentisering, vil Blackbox AI be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `blackbox ai mcp auth <server-name>`.
...
```json title="blackbox ai.json" {7-11}
{
  "$schema": "https://www.blackbox.ai/config.json",
  "mcp": {
    "my-oauth-server": {
...
Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil Blackbox AI lagre tokens sikkert i `~/.local/share/blackbox ai/mcp-auth.json`.
...
MCP-ene dine er tilgjengelige som verktøy i Blackbox AI, sammen med innebygde verktøy. Så du kan administrere dem gjennom Blackbox AI-konfigurasjonen som alle andre verktøy.
...
```json title="blackbox ai.json" {14}
{
  "$schema": "https://www.blackbox.ai/config.json",
  "mcp": {
    "my-mcp-foo": {
...
```json title="blackbox ai.json" {11, 14-18}
{
  "$schema": "https://www.blackbox.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```json title="blackbox ai.json" {4-8}
{
  "$schema": "https://www.blackbox.ai/config.json",
  "mcp": {
    "sentry": {
...
Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble Blackbox AI til Sentry-kontoen din.
...
```json title="blackbox ai.json" {4-7}
{
  "$schema": "https://www.blackbox.ai/config.json",
  "mcp": {
    "context7": {
...
```json title="blackbox ai.json" {7-9}
{
  "$schema": "https://www.blackbox.ai/config.json",
  "mcp": {
    "context7": {
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver Blackbox AI-samtale oppført som en separat økt.
