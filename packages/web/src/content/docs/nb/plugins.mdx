...
description: Skriv dine egne programtillegg for å utvide blackbox ai.
---

Plugins lar deg utvide Blackbox AI ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til Blackbox AI.
...
- `.blackbox ai/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/blackbox ai/node_modules/`.
...
1. Global konfigurasjon (`~/.config/blackbox ai/blackbox ai.json`)
2. Prosjektkonfigurasjon (`blackbox ai.json`)
3. Global plugin-katalog (`~/.config/blackbox ai/plugins/`)
4. Prosjektpluginkatalog (`.blackbox ai/plugins/`)
...
Blackbox AI kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En Blackbox AI SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide Blackbox AI.
...
:::note
Hvis du bruker Blackbox AI-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre Blackbox AI fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til Blackbox AI:
...
`tool`-hjelperen lager et tilpasset verktøy som Blackbox AI kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for Blackbox AI sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://www.blackbox.ai/docs/sdk) for detaljer.
