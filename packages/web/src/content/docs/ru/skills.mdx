...
Навыки агента позволяют blackbox ai обнаруживать многократно используемые инструкции из вашего репозитория или домашнего каталога.
...
blackbox ai выполняет поиск в следующих местах:

- Конфигурация проекта: `.blackbox ai/skills/<name>/SKILL.md`
- Глобальная конфигурация: `~/.config/blackbox ai/skills/<name>/SKILL.md`.
...
Для локальных путей проекта blackbox ai переходит из вашего текущего рабочего каталога, пока не достигнет рабочего дерева git.
Он загружает все соответствующие `skills/*/SKILL.md` в `.blackbox ai/` и все соответствующие `.claude/skills/*/SKILL.md` или `.agents/skills/*/SKILL.md` по пути.

Глобальные определения также загружаются из `~/.config/blackbox ai/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` и `~/.agents/skills/*/SKILL.md`.
...
Создайте `.blackbox ai/skills/git-release/SKILL.md` следующим образом:
...
blackbox ai перечисляет доступные навыки в описании инструмента `skill`.
...
Контролируйте, к каким навыкам агенты могут получить доступ, используя разрешения на основе шаблонов в `blackbox ai.json`:
...
**Для встроенных агентов** (в формате `blackbox ai.json`):
