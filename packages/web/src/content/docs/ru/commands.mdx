...
Создайте `.blackbox ai/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию blackbox ai или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем blackbox ai [config](/docs/config):
...
- Глобальный: `~/.config/blackbox ai/commands/`
- Для каждого проекта: `.blackbox ai/commands/`
...
blackbox ai включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
