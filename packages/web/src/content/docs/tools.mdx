...
Tools allow the LLM to perform actions in your codebase. Blackbox AI comes with a set of built-in tools, but you can extend it with [custom tools](/docs/custom-tools) or [MCP servers](/docs/mcp-servers).
...
Here are all the built-in tools available in Blackbox AI.
...
:::note
This tool is only available when `BLACKBOX_AI_EXPERIMENTAL_LSP_TOOL=true` (or `BLACKBOX_AI_EXPERIMENTAL=true`).
:::
...
:::note
This tool is only available when using the Blackbox AI provider or when the `BLACKBOX_AI_ENABLE_EXA` environment variable is set to any truthy value (e.g., `true` or `1`).

To enable when launching Blackbox AI:
