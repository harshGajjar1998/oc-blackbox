...
Create `.blackbox ai/commands/test.md`:
...
You can add custom commands through the Blackbox AI config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your Blackbox AI [config](/docs/config):
...
- Per-project: `.blackbox ai/commands/`
...
blackbox ai includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
