---
title: サーバー
description: HTTP 経由で Blackbox AI サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`blackbox ai serve` コマンドは、Blackbox AI クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `blackbox ai.local` |
...
HTTP 基本認証でサーバーを保護するには、`BLACKBOX_AI_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `blackbox ai` になるか、`BLACKBOX_AI_SERVER_USERNAME` を設定してオーバーライドします。これは、`blackbox ai serve` と `blackbox ai web` の両方に当てはまります。
...
`blackbox ai` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
Blackbox AI サーバーを使用して、プログラムで Blackbox AI と対話します。
:::

`blackbox ai serve` を実行してスタンドアロンサーバーを起動できます。すでに Blackbox AI TUI を実行している場合でも、`blackbox ai serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、Blackbox AI [IDE](/docs/ide) プラグインによって使用されます。
...
Blackbox AI サーバーは次の API を公開します。
