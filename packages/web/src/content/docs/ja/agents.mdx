...
Blackbox AI には 2 種類のエージェントがあります。プライマリエージェントとサブエージェント。
...
Blackbox AI には、**Build** と **Plan** という 2 つの組み込みプライマリエージェントが付属しています。これらについて以下で説明します。
...
Blackbox AI には、**General** と **Explore** という 2 つの組み込みサブエージェントが付属しています。これについては以下で見ていきます。

---

## 組み込み

Blackbox AI には、2 つの組み込みプライマリエージェントと 2 つの組み込みサブエージェントが付属しています。
...
`blackbox ai.json` 設定ファイルでエージェントを構成します。
...
- グローバル: `~/.config/blackbox ai/agents/`
- プロジェクトごと: `.blackbox ai/agents/`
...
温度が指定されていない場合、Blackbox AI はモデル固有のデフォルトを使用します。通常、ほとんどのモデルでは 0、Qwen モデルでは 0.55 です。
...
このパスは、設定ファイルが配置されている場所に対する相対パスです。したがって、これはグローバルな Blackbox AI 設定とプロジェクト固有の設定の両方で機能します。
...
Blackbox AI 設定内のモデル ID は、`provider/model-id` という形式を使用します。たとえば、[Blackbox AI Zen](/docs/zen) を使用している場合、GPT 5.1 Codex には `blackbox ai/gpt-5.1-codex` を使用します。
...
:::tip
`blackbox ai models` を実行して、利用可能なモデルのリストを表示します。
:::
...
:::tip
共有したいエージェントはいますか? [PR](https://github.com/anomalyco/blackbox ai) を送信します。
:::
