...
:::caution
模式現在透過 blackbox ai 設定中的 `agent` 選項進行設定。`mode` 選項已廢棄。[了解更多](/docs/agents)。
:::

blackbox ai 中的模式允許您為不同的使用情境自訂行為、工具和提示詞。

blackbox ai 自帶兩種內建模式：**build** 和 **plan**。您可以自訂這些模式，也可以透過 blackbox ai 設定建立自己的模式。
...
- `write` - 無法建立新檔案
- `edit` - 無法修改現有檔案，但位於 `.blackbox ai/plans/*.md` 的檔案除外，用於詳細說明計畫本身
- `patch` - 無法套用補丁
- `bash` - 無法執行 shell 指令
...
- 全域：`~/.config/blackbox ai/modes/`
- 專案：`.blackbox ai/modes/`
...
此路徑相對於設定檔所在位置。因此，全域 blackbox ai 設定和專案特定設定均可使用。
...
在 `.blackbox ai/modes/` 中建立專案特定的模式檔案，或在 `~/.config/blackbox ai/modes/` 中建立全域模式檔案：
