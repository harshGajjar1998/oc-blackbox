---
title: Custom Tools
description: Create tools the LLM can call in blackbox ai.
---

Custom tools are functions you create that the LLM can call during conversations. They work alongside blackbox ai's [built-in tools](/docs/tools) like `read`, `write`, and `bash`.
...
- Locally by placing them in the `.blackbox ai/tools/` directory of your project.
- Or globally, by placing them in `~/.config/blackbox ai/tools/`.
...
```ts title=".blackbox ai/tools/bash.ts"
import { tool } from "@blackbox ai-ai/plugin"
