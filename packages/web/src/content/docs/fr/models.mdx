...
Les fournisseurs les plus populaires sont préchargés par défaut. Si vous avez ajouté les informations d'identification d'un fournisseur via la commande `/connect`, elles seront disponibles lorsque vous démarrerez Blackbox AI.
...
Voici plusieurs modèles qui fonctionnent bien avec Blackbox AI, sans ordre particulier. (Cette liste n’est pas exhaustive et n’est pas nécessairement à jour) :
...
Pour définir l'un d'entre eux comme modèle par défaut, vous pouvez définir la clé `model` dans votre configuration Blackbox AI.

```json title="blackbox ai.json" {3}
{
...
Ici, l'identifiant complet est `provider_id/model_id`. Par exemple, si vous utilisez [Blackbox AI Zen](/docs/zen), vous utiliserez `blackbox ai/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```jsonc title="blackbox ai.jsonc" {7-12,19-24}
{
  "$schema": "https://www.blackbox.ai/config.json",
  "provider": {
    "openai": {
...
```jsonc title="blackbox ai.jsonc" {6-21}
{
  "$schema": "https://www.blackbox.ai/config.json",
  "provider": {
    "blackbox ai": {
...
De nombreux modèles prennent en charge plusieurs variantes avec différentes configurations. Blackbox AI est livré avec des variantes par défaut intégrées pour les fournisseurs populaires.

### Variantes intégrées

Blackbox AI est livré avec des variantes par défaut pour de nombreux fournisseurs :
...
```jsonc title="blackbox ai.jsonc" {7-18}
{
  "$schema": "https://www.blackbox.ai/config.json",
  "provider": {
    "openai": {
...
Lorsque Blackbox AI démarre, il recherche les modèles dans l'ordre de priorité suivant :
...
2. La liste des modèles dans la configuration Blackbox AI.
