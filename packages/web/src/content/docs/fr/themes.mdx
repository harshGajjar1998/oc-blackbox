...
Avec Blackbox AI, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, Blackbox AI utilise notre propre thème `blackbox ai`.
...
Blackbox AI est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que Blackbox AI corresponde à l'apparence de leur terminal
...
Blackbox AI prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/blackbox ai/themes/*.json` ou `$XDG_CONFIG_HOME/blackbox ai/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.blackbox ai/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.blackbox ai/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/blackbox ai/themes
vim ~/.config/blackbox ai/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .blackbox ai/themes
vim .blackbox ai/themes/my-theme.json
```
...
{
  "$schema": "https://www.blackbox.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
