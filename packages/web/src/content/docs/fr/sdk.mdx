---
title: SDK
description: Client JS de type sécurisé pour le serveur blackbox ai.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK blackbox ai JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler blackbox ai par programme.
...
```bash
npm install @blackbox ai-ai/sdk
```
...
Créez une instance de blackbox ai :

```javascript
import { createBlackbox AI } from "@blackbox ai-ai/sdk"

const { client } = await createBlackbox AI()
```
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `blackbox ai.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :

```javascript
import { createBlackbox AI } from "@blackbox ai-ai/sdk"

const blackbox ai = await createBlackbox AI({
  hostname: "127.0.0.1",
  port: 4096,
  config: {
...
console.log(`Server running at ${blackbox ai.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de blackbox ai, vous pouvez créer une instance client pour vous y connecter :

```javascript
import { createBlackbox AIClient } from "@blackbox ai-ai/sdk"

const client = createBlackbox AIClient({
...
```typescript
import type { Session, Message, Part } from "@blackbox ai-ai/sdk"
```
...
const textResults = await client.find.text({
  query: { pattern: "function.*blackbox ai" },
})
