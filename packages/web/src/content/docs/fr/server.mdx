---
title: Serveur
description: Interagissez avec le serveur blackbox ai via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `blackbox ai serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client blackbox ai peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `blackbox ai.local` |
...
Définissez `BLACKBOX_AI_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `blackbox ai`, ou définissez `BLACKBOX_AI_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `blackbox ai serve` et à `blackbox ai web`.
...
Lorsque vous exécutez `blackbox ai`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur blackbox ai pour interagir avec blackbox ai par programmation.
:::

Cette architecture permet à blackbox ai de prendre en charge plusieurs clients et vous permet d'interagir avec blackbox ai par programme.

Vous pouvez exécuter `blackbox ai serve` pour démarrer un serveur autonome. Si vous avez le
blackbox ai TUI en cours d'exécution, `blackbox ai serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins Blackbox AI [IDE](/docs/ide).
...
Le serveur blackbox ai expose les API suivantes.
