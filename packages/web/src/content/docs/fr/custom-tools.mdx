...
description: Créez des outils que le LLM peut appeler dans blackbox ai.
---

Les outils personnalisés sont des fonctions que vous créez et que le LLM peut appeler pendant les conversations. Ils fonctionnent avec les [outils intégrés](/docs/tools) de blackbox ai comme `read`, `write` et `bash`.
...
- Localement en les plaçant dans le répertoire `.blackbox ai/tools/` de votre projet.
- Ou globalement, en les plaçant dans `~/.config/blackbox ai/tools/`.
...
```ts title=".blackbox ai/tools/database.ts" {1}
import { tool } from "@blackbox ai-ai/plugin"
...
```ts title=".blackbox ai/tools/math.ts"
import { tool } from "@blackbox ai-ai/plugin"
...
```ts title=".blackbox ai/tools/project.ts" {8}
import { tool } from "@blackbox ai-ai/plugin"
...
```python title=".blackbox ai/tools/add.py"
import sys
...
```ts title=".blackbox ai/tools/python-add.ts" {10}
import { tool } from "@blackbox ai-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".blackbox ai/tools/add.py")
