...
Il existe deux types d'agents dans Blackbox AI ; agents primaires et sous-agents.
...
Blackbox AI est livré avec deux agents primaires intégrés, **Build** et **Plan**.
...
Blackbox AI est livré avec deux sous-agents intégrés, **General** et **Explore**. Nous verrons cela ci-dessous.

---

## Agents intégrés

Blackbox AI est livré avec deux agents primaires intégrés et deux sous-agents intégrés.
...
Configurez les agents dans votre fichier de configuration `blackbox ai.json` :
...
- Global : `~/.config/blackbox ai/agents/`
- Par projet : `.blackbox ai/agents/`

```markdown title="~/.config/blackbox ai/agents/review.md"
---
...
Si aucune température n'est spécifiée, Blackbox AI utilise les valeurs par défaut spécifiques au modèle ; généralement 0 pour la plupart des modèles, 0,55 pour les modèles Qwen.
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Cela fonctionne donc à la fois pour la configuration globale Blackbox AI et pour la configuration spécifique au projet.
...
L'ID de modèle dans votre configuration Blackbox AI utilise le format `provider/model-id`. Par exemple, si vous utilisez [Blackbox AI Zen](/docs/zen), vous utiliserez `blackbox ai/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```json title="blackbox ai.json" {3-6,9-12}
{
  "$schema": "https://www.blackbox.ai/config.json",
  "tools": {
    "write": true,
...
```json title="blackbox ai.json" {3-5,8-10}
{
  "$schema": "https://www.blackbox.ai/config.json",
  "permission": {
    "edit": "deny"
...
```json title="blackbox ai.json" {6,7}
{
  "agent": {
    "deep-thinker": {
...
:::tip
Exécutez `blackbox ai models` pour voir une liste des modèles disponibles.
:::
...
:::tip
Avez-vous un agent que vous aimeriez partager ? [Soumettez un PR](https://github.com/anomalyco/blackbox ai).
:::
...
```markdown title="~/.config/blackbox ai/agents/docs-writer.md"
---
...
```markdown title="~/.config/blackbox ai/agents/security-auditor.md"
---
