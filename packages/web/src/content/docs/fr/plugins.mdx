---
title: Plugins
description: Écrivez vos propres plugins pour étendre Blackbox AI.
---

Les plugins vous permettent d'étendre Blackbox AI en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de Blackbox AI.
...
- `.blackbox ai/plugins/` - Plugins au niveau du projet
- `~/.config/blackbox ai/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/blackbox ai/node_modules/`.
...
1. Configuration globale (`~/.config/blackbox ai/blackbox ai.json`)
2. Configuration du projet (`blackbox ai.json`)
3. Répertoire global des plugins (`~/.config/blackbox ai/plugins/`)
4. Répertoire des plugins du projet (`.blackbox ai/plugins/`)
...
Blackbox AI exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK blackbox ai pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre blackbox ai.
...
:::note
Si vous utilisez l'application de bureau Blackbox AI, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez blackbox ai de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à blackbox ai :
...
L'assistant `tool` crée un outil personnalisé que blackbox ai peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour blackbox ai aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://www.blackbox.ai/docs/sdk) pour plus de détails.
