---
title: Fournisseurs
description: Utiliser n’importe quel fournisseur LLM en Blackbox AI.
---

import config from "../../../../config.mjs"
export const console = config.console

Blackbox AI utilise [AI SDK](https://ai-sdk.dev/) et [Models.dev](https://models.dev) pour prendre en charge **75+ fournisseurs LLM** et prend en charge l'exécution de modèles locaux.

Pour ajouter un fournisseur, vous devez :

1. Ajoutez les clés API pour le fournisseur à l'aide de la commande `/connect`.
2. Configurez le fournisseur dans votre configuration Blackbox AI.
...
en `~/.local/share/blackbox ai/auth.json`.

---

### Configuration

Vous pouvez personnaliser les fournisseurs via la section `provider` de votre Blackbox AI
configuration.
...
Blackbox AI Zen est une liste de modèles fournis par l'équipe Blackbox AI qui ont été
testé et vérifié pour fonctionner correctement avec Blackbox AI. [En savoir plus](/docs/zen).

:::tip
Si vous êtes nouveau, nous vous recommandons de commencer par Blackbox AI Zen.
:::

1. Exécutez la commande `/connect` dans le TUI, sélectionnez blackbox ai et dirigez-vous vers [blackbox.ai/auth](https://www.blackbox.ai/auth).
...
Il fonctionne comme n’importe quel autre fournisseur dans Blackbox AI et son utilisation est totalement facultative.
...
Pour utiliser Amazon Bedrock avec Blackbox AI :
...
Définissez l'une de ces variables d'environnement lors de l'exécution de blackbox ai :

```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY blackbox ai

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile blackbox ai

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX blackbox ai
```
...
Pour une configuration spécifique au projet ou persistante, utilisez `blackbox ai.json` :
...
:::info
L'utilisation de votre abonnement Claude Pro/Max dans Blackbox AI n'est pas officiellement prise en charge par [Anthropic](https://anthropic.com).
:::
...
2. Accédez à [Azure AI Foundry](https://ai.azure.com/) et déployez un modèle.
   ...
   :::note
   Le nom du déploiement doit correspondre au nom du modèle pour que blackbox ai fonctionne correctement.
   :::
...
5. Définissez le nom de votre ressource comme variable d'environnement :
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX blackbox ai
   ```
...
5. Définissez le nom de votre ressource comme variable d'environnement :
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX blackbox ai
   ```
...
Vous pouvez également ajouter des modèles via votre configuration blackbox ai.
...
pour stocker le jeton dans le stockage d'authentification blackbox ai.
:::

##### GitLab auto-hébergé

:::note[note de conformité]
Blackbox AI utilise un petit modèle pour certaines tâches d'IA telles que la génération du titre de la session.
Il est configuré pour utiliser gpt-5-nano par défaut, hébergé par Zen. Pour verrouiller Blackbox AI
pour utiliser uniquement votre propre instance hébergée par GitLab, ajoutez ce qui suit à votre
Fichier `blackbox ai.json`. Il est également recommandé de désactiver le partage de session.
...
Plus de documentation sur la page d'accueil [blackbox ai-gitlab-auth](https://www.npmjs.com/package/@gitlab/blackbox ai-gitlab-auth).

##### Configuration

Personnalisez via `blackbox ai.json` :
...
{
  "$schema": "https://www.blackbox.ai/config.json",
  "plugin": ["@gitlab/blackbox ai-gitlab-plugin"]
}
```
...
Pour utiliser votre abonnement GitHub Copilot avec blackbox ai :
...
Pour utiliser Google Vertex AI avec Blackbox AI :
...
Définissez-les lors de l'exécution de blackbox ai.

```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id blackbox ai
```
...
Si vous voyez une fonctionnalité ou un modèle d'Helicone qui n'est pas configuré automatiquement via blackbox ai, vous pouvez toujours le configurer vous-même.
...
```jsonc title="~/.config/blackbox ai/blackbox ai.jsonc"
{
  "$schema": "https://www.blackbox.ai/config.json",
  "provider": {
    "helicone": {
...
La fonctionnalité [Sessions](https://docs.helicone.ai/features/sessions) d'Helicone vous permet de regrouper les requêtes LLM associées. Utilisez le plugin [blackbox ai-helicone-session](https://github.com/H2Shami/blackbox ai-helicone-session) pour enregistrer automatiquement chaque conversation Blackbox AI en tant que session dans Helicone.

```bash
npm install -g blackbox ai-helicone-session
```
...
```json title="blackbox ai.json"
{
  "plugin": ["blackbox ai-helicone-session"]
}
```

Le plugin injecte les en-têtes `Helicone-Session-Id` et `Helicone-Session-Name` dans vos requêtes. Sur la page Sessions d'Helicone, vous verrez chaque conversation Blackbox AI répertoriée comme une session distincte.
...
Vous pouvez configurer blackbox ai pour utiliser des modèles locaux via l'utilitaire llama-server de [llama.cpp's](https://github.com/ggml-org/llama.cpp)

```json title="blackbox ai.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://www.blackbox.ai/config.json",
  "provider": {
    "llama.cpp": {
...
Vous pouvez configurer blackbox ai pour utiliser des modèles locaux via LM Studio.

```json title="blackbox ai.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://www.blackbox.ai/config.json",
  "provider": {
    "lmstudio": {
...
Vous pouvez configurer blackbox ai pour utiliser des modèles locaux via Ollama.

:::tip
Ollama peut se configurer automatiquement pour Blackbox AI. Voir les [documents d'intégration Ollama](https://docs.ollama.com/integrations/blackbox ai) pour plus de détails.
:::

```json title="blackbox ai.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://www.blackbox.ai/config.json",
  "provider": {
    "ollama": {
...
Pour utiliser Ollama Cloud avec Blackbox AI :
...
3. Copiez la clé API à utiliser dans Blackbox AI.
...
6. **Important** : Avant d'utiliser des modèles cloud dans Blackbox AI, vous devez extraire les informations du modèle localement :
...
Blackbox AI Zen est une liste de modèles testés et vérifiés fournie par l'équipe Blackbox AI. [En savoir plus](/docs/zen).

1. Connectez-vous à **<a href={console}>Blackbox AI Zen</a>** et cliquez sur **Créer une clé API**.

2. Exécutez la commande `/connect` et recherchez **Blackbox AI Zen**.

   ```txt
   /connect
   ```

3. Entrez votre clé Blackbox AI API.
...
Vous pouvez également ajouter des modèles supplémentaires via votre configuration blackbox ai.
...
5. Vous pouvez également les personnaliser via votre configuration blackbox ai. Voici un exemple de spécification d'un fournisseur
...
```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' blackbox ai
```
...
4. Définissez éventuellement l'ID de déploiement et le groupe de ressources :
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group blackbox ai
   ```
...
Pour utiliser [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) avec Blackbox AI :
...
Vous pouvez également personnaliser les modèles via votre configuration blackbox ai. Voici un exemple de spécification de l'ordre de routage du fournisseur.
...
:::tip
Vous pouvez utiliser n'importe quel fournisseur compatible OpenAI avec blackbox ai. La plupart des fournisseurs d'IA modernes proposent des API compatibles OpenAI.
:::
...
3. Entrez votre clé API pour le fournisseur.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in blackbox ai.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Créez ou mettez à jour votre fichier `blackbox ai.json` dans le répertoire de votre projet :

   ```json title="blackbox ai.json" ""myprovider"" {5-15}
   {
     "$schema": "https://www.blackbox.ai/config.json",
     "provider": {
       "myprovider": {
...
```json title="blackbox ai.json" {9,11,17-20}
{
  "$schema": "https://www.blackbox.ai/config.json",
  "provider": {
    "myprovider": {
...
Les champs `limit` permettent à Blackbox AI de comprendre la quantité de contexte qu'il vous reste. Les fournisseurs standard les extraient automatiquement de models.dev.
...
1. **Vérifiez la configuration de l'authentification** : exécutez `blackbox ai auth list` pour voir si les informations d'identification
...
2. Pour les fournisseurs personnalisés, vérifiez la configuration blackbox ai et :
   - Assurez-vous que l'ID du fournisseur utilisé dans la commande `/connect` correspond à l'ID de votre configuration blackbox ai.
