...
Les compétences d'agent permettent à Blackbox AI de découvrir des instructions réutilisables à partir de votre dépôt ou de votre répertoire personnel.
...
Blackbox AI recherche ces emplacements :

- Configuration du projet : `.blackbox ai/skills/<name>/SKILL.md`
- Configuration globale : `~/.config/blackbox ai/skills/<name>/SKILL.md`
...
Pour les chemins locaux du projet, Blackbox AI remonte de votre répertoire de travail actuel jusqu'à ce qu'il atteigne l'arbre de travail git.
Il charge tout `skills/*/SKILL.md` correspondant dans `.blackbox ai/` et tout `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` correspondant en cours de route.

Les définitions globales sont également chargées à partir de `~/.config/blackbox ai/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` et `~/.agents/skills/*/SKILL.md`.
...
Créez `.blackbox ai/skills/git-release/SKILL.md` comme ceci :
...
Blackbox AI répertorie les compétences disponibles dans la description de l'outil `skill`.
...
Contrôlez les compétences auxquelles les agents peuvent accéder à l'aide d'autorisations basées sur des modèles dans `blackbox ai.json` :
...
**Pour les agents intégrés** (dans `blackbox ai.json`) :
