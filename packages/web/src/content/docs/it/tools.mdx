...
Gli strumenti permettono all'LLM di eseguire azioni nel tuo codebase. Blackbox AI include un set di strumenti integrati, ma puoi estenderlo con [strumenti personalizzati](/docs/custom-tools) o con [server MCP](/docs/mcp-servers).
...
Ecco tutti gli strumenti integrati disponibili in Blackbox AI.
...
:::note
Questo strumento e' disponibile solo quando `BLACKBOX_AI_EXPERIMENTAL_LSP_TOOL=true` (oppure `BLACKBOX_AI_EXPERIMENTAL=true`).
:::
...
:::note
Questo strumento e' disponibile solo quando usi il provider Blackbox AI o quando la variabile d'ambiente `BLACKBOX_AI_ENABLE_EXA` e' impostata a un valore truthy (ad esempio `true` o `1`).

Per abilitarlo all'avvio di Blackbox AI:
