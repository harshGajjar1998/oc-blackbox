---
title: Provider
description: Usa qualsiasi provider LLM in Blackbox AI.
---

import config from "../../../../config.mjs"
export const console = config.console

Blackbox AI usa [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) per supportare **oltre 75 provider LLM** e anche modelli locali.

Per aggiungere un provider devi:

1. Aggiungere le API key del provider con il comando `/connect`.
2. Configurare il provider nella configurazione di Blackbox AI.
...
Blackbox AI Zen e una lista di modelli forniti dal team Blackbox AI, testati e
verificati per funzionare bene con Blackbox AI. [Scopri di piu](/docs/zen).

:::tip
Se sei all'inizio, ti consigliamo di partire da Blackbox AI Zen.
:::

1. Esegui `/connect` nella TUI, seleziona blackbox ai e vai su [blackbox.ai/auth](https://www.blackbox.ai/auth).
...
Funziona come qualsiasi altro provider in Blackbox AI ed e totalmente opzionale.
...
Per usare Amazon Bedrock con Blackbox AI:
...
2. **Configura l'autenticazione** usando uno dei seguenti metodi:
   ...
   ***
   ...
   Imposta una di queste variabili d'ambiente mentre esegui blackbox ai:
   ...
   ***
   ...
   Per configurazione specifica del progetto o persistente, usa `blackbox ai.json`:
...
:::info
L'uso dell'abbonamento Claude Pro/Max in Blackbox AI non è ufficialmente supportato da [Anthropic](https://anthropic.com).
:::
...
2. Vai su [Azure AI Foundry](https://ai.azure.com/) e fai il deploy di un modello.
   ...
   :::note
   Il nome del deployment deve corrispondere al nome del modello affinché blackbox ai funzioni correttamente.
   :::
...
5. Esegui il comando `/models` per selezionare un modello.
   ...
   Puoi anche aggiungere modelli tramite la tua configurazione di blackbox ai.
...
salvare il token nello storage di auth di blackbox ai.
:::

##### GitLab Self-Hosted

:::note[nota di compliance]
Blackbox AI usa un modello piccolo per alcuni task AI come generare il titolo della sessione.
È configurato per usare gpt-5-nano di default, ospitato da Zen. Per bloccare Blackbox AI
a usare solo la tua istanza GitLab self-hosted, aggiungi quanto segue al tuo
file `blackbox ai.json`. Si raccomanda anche di disabilitare la condivisione sessioni.
...
Maggior documentazione sulla homepage di [blackbox ai-gitlab-auth](https://www.npmjs.com/package/@gitlab/blackbox ai-gitlab-auth).

##### Configurazione

Personalizza tramite `blackbox ai.json`:
...
Per usare il tuo abbonamento GitHub Copilot con blackbox ai:
...
Per usare Google Vertex AI con Blackbox AI:
...
2. Imposta le variabili d'ambiente richieste:
   ...
   Impostale mentre esegui blackbox ai.
...
Nel caso vedessi una funzionalità o un modello di Helicone che non viene configurato automaticamente da blackbox ai, puoi sempre configurarlo tu stesso.
...
La funzionalità [Sessions](https://docs.helicone.ai/features/sessions) di Helicone ti permette di raggruppare richieste LLM correlate. Usa il plugin [blackbox ai-helicone-session](https://github.com/H2Shami/blackbox ai-helicone-session) per loggare automaticamente ogni conversazione di Blackbox AI come una sessione in Helicone.
...
Il plugin inietta gli header `Helicone-Session-Id` e `Helicone-Session-Name` nelle tue richieste. Nella pagina Sessions di Helicone, vedrai ogni conversazione di Blackbox AI elencata come sessione separata.
...
Puoi configurare blackbox ai per usare modelli locali tramite l'utility llama-server di [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puoi configurare blackbox ai per usare modelli locali tramite LM Studio.
...
Puoi configurare blackbox ai per usare modelli locali tramite Ollama.

:::tip
Ollama può auto-configurarsi per Blackbox AI. Vedi la [documentazione di integrazione Ollama](https://docs.ollama.com/integrations/blackbox ai) per dettagli.
:::
...
Per usare Ollama Cloud con Blackbox AI:
...
3. Copia la chiave API da usare in Blackbox AI.
...
6. **Importante**: Prima di usare modelli cloud in Blackbox AI, devi scaricare le informazioni del modello localmente:
...
Blackbox AI Zen è una lista di modelli testati e verificati forniti dal team Blackbox AI. [Scopri di più](/docs/zen).

1. Accedi a **<a href={console}>Blackbox AI Zen</a>** e clicca **Create API Key**.

2. Esegui il comando `/connect` e cerca **Blackbox AI Zen**.

   ```txt
   /connect
   ```

3. Inserisci la tua chiave API di Blackbox AI.
...
4. Molti modelli OpenRouter sono precaricati di default, esegui il comando `/models` per selezionare quello che vuoi.
   ...
   Puoi anche aggiungere modelli addizionali tramite la tua configurazione di blackbox ai.
...
5. Puoi anche personalizzarli tramite la configurazione di blackbox ai. Ecco un esempio di specifica di un provider
...
Per usare le [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) con Blackbox AI:
...
Puoi anche personalizzare i modelli tramite la tua configurazione di blackbox ai. Ecco un esempio di come specificare l'ordine di routing dei provider.
...
:::tip
Puoi usare qualsiasi provider OpenAI-compatible con blackbox ai. La maggior parte dei provider AI moderni offre API OpenAI-compatible.
:::
...
4. Crea o aggiorna il tuo file `blackbox ai.json` nella directory del progetto:
...
I campi `limit` permettono a Blackbox AI di capire quanto contesto rimane. I provider standard recuperano questi dati da models.dev automaticamente.
...
1. **Controlla il setup auth**: Esegui `blackbox ai auth list` per vedere se le credenziali
...
2. Per provider personalizzati, controlla la config di blackbox ai e:
   - Assicurati che l'ID provider usato nel comando `/connect` corrisponda all'ID nella tua config blackbox ai.
