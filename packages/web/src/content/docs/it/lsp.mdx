---
title: Server LSP
description: Blackbox AI si integra con i tuoi server LSP.
---

Blackbox AI si integra con il tuo Language Server Protocol (LSP) per aiutare l'LLM a interagire con la tua codebase. Usa le diagnostiche per fornire feedback all'LLM.

---

## Integrati

Blackbox AI include diversi server LSP integrati per linguaggi popolari:
...
:::note
Puoi disabilitare il download automatico dei server LSP impostando la variabile d'ambiente `BLACKBOX_AI_DISABLE_LSP_DOWNLOAD` a `true`.
:::
...
Quando blackbox ai apre un file:
...
Puoi personalizzare i server LSP tramite la sezione `lsp` nella configurazione di blackbox ai.
