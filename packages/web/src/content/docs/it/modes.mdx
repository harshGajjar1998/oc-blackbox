...
:::caution
Le modalita ora si configurano tramite l'opzione `agent` nella configurazione di blackbox ai. L'opzione
`mode` e ora deprecata. [Scopri di piu](/docs/agents).
:::

Le modalita in blackbox ai ti permettono di personalizzare comportamento, strumenti e prompt per casi d'uso diversi.

Include due modalita integrate: **build** e **plan**. Puoi personalizzarle oppure configurarne di tue tramite la configurazione di blackbox ai.
...
blackbox ai include due modalita integrate.
...
- `edit` - Non puo modificare file esistenti, tranne i file in `.blackbox ai/plans/*.md` per dettagliare il piano
...
Configura le modalita nel file di configurazione `blackbox ai.json`:
...
- Globale: `~/.config/blackbox ai/modes/`
- Progetto: `.blackbox ai/modes/`
...
Se non viene specificata alcuna temperatura, blackbox ai usa i default specifici del modello (tipicamente 0 per la maggior parte dei modelli, 0.55 per i modelli Qwen).
...
sia per la configurazione globale di blackbox ai sia per quella specifica del progetto.
...
Crea file di modalita in `.blackbox ai/modes/` per modalita specifiche del progetto oppure in `~/.config/blackbox ai/modes/` per modalita globali:
