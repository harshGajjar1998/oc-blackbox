...
Per diagnosticare problemi con Blackbox AI, inizia controllando i log e i dati locali che salva su disco.
...
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\blackbox ai\log`

I file di log hanno nomi con timestamp (ad es. `2025-01-09T123456.log`) e vengono conservati i 10 file di log piu recenti.

Puoi impostare il livello di log con l'opzione a riga di comando `--log-level` per ottenere informazioni di debug piu dettagliate. Per esempio: `blackbox ai --log-level DEBUG`.

---

## Archiviazione

blackbox ai salva i dati delle sessioni e altri dati dell'applicazione su disco in:

- **macOS/Linux**: `~/.local/share/blackbox ai/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\blackbox ai`
...
Blackbox AI Desktop esegue in background un server locale di Blackbox AI (il sidecar `blackbox ai-cli`). La maggior parte dei problemi e causata da un plugin che si comporta male, da una cache corrotta o da un'impostazione del server errata.
...
- Solo macOS: menu `Blackbox AI` -> **Reload Webview** (utile se l'interfaccia e vuota o bloccata).
...
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\blackbox ai\blackbox ai.jsonc`
...
Blackbox AI puo anche caricare plugin locali dal disco. Spostali temporaneamente altrove (o rinomina la cartella) e riavvia l'app desktop:

- **Plugin globali**
  - **macOS/Linux**: `~/.config/blackbox ai/plugins/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\blackbox ai\plugins`
...
Se disabilitare i plugin non aiuta (o l'installazione di un plugin e bloccata), svuota la cache in modo che Blackbox AI possa ricostruirla.

1. Quit Blackbox AI Desktop completely.
2. Elimina la directory della cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/blackbox ai`
- **Linux**: elimina `~/.cache/blackbox ai` (oppure esegui `rm -rf ~/.cache/blackbox ai`)
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.cache\blackbox ai`
...
Blackbox AI Desktop puo avviare il proprio server locale (predefinito) oppure connettersi a un URL server che hai configurato.
...
Se il tuo `blackbox ai.json(c)` contiene una sezione `server`, rimuovila temporaneamente e riavvia l'app desktop.

#### Controlla le variabili d'ambiente

Se hai `BLACKBOX_AI_PORT` impostato nell'ambiente, l'app desktop provera a usare quella porta per il server locale.

- Rimuovi `BLACKBOX_AI_PORT` (o scegli una porta libera) e riavvia.
...
Su Windows, Blackbox AI Desktop richiede **WebView2 Runtime** di Microsoft Edge. Se l'app si apre su una finestra vuota o non parte, installa/aggiorna WebView2 e riprova.

---

### Windows: problemi generali di prestazioni

Se riscontri prestazioni lente, problemi di accesso ai file o problemi del terminale su Windows, prova a usare [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL fornisce un ambiente Linux che funziona in modo piu fluido con le funzionalita di Blackbox AI.

---

### Notifiche non visualizzate

Blackbox AI Desktop mostra le notifiche di sistema solo quando:

- le notifiche sono abilitate per Blackbox AI nelle impostazioni del sistema operativo, e
...
2. Trova ed elimina questi file (si trovano nella directory dati dell'app Blackbox AI Desktop):

- `blackbox ai.settings.dat` (desktop default server URL)
- `blackbox ai.global.dat` e `blackbox ai.workspace.*.dat` (stato dell'interfaccia come server/progetti recenti)
...
Se riscontri problemi con Blackbox AI:
...
Per capire a quali modelli hai accesso, esegui `blackbox ai models`
...
2. Se il problema persiste, prova a cancellare la configurazione salvata:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.local\share\blackbox ai`
...
Se incontri errori nelle chiamate API, potrebbe dipendere da pacchetti provider non aggiornati. blackbox ai installa dinamicamente i pacchetti provider (OpenAI, Anthropic, Google, ecc.) quando servono e li mette in cache localmente.
...
1. Svuota la cache dei pacchetti provider:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.cache\blackbox ai`

2. Riavvia blackbox ai per reinstallare i pacchetti provider piu recenti

Questo forzera blackbox ai a scaricare le versioni piu recenti dei pacchetti provider, cosa che spesso risolve problemi di compatibilita con parametri dei modelli e cambiamenti delle API.
...
blackbox ai rilevera se stai usando Wayland e preferira `wl-clipboard`; altrimenti provera a trovare gli strumenti per gli appunti nell'ordine: `xclip` e `xsel`.
