...
In Blackbox AI esistono due tipi di agenti: agenti primari e subagenti.
...
Blackbox AI include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
Blackbox AI include due subagenti integrati: **General** e **Explore**. Li vediamo sotto.

---

## Integrati

Blackbox AI include due agenti primari integrati e due subagenti integrati.
...
Configura gli agenti nel file `blackbox ai.json`:
...
- Globale: `~/.config/blackbox ai/agents/`
- Per progetto: `.blackbox ai/agents/`
...
Se non specifichi una temperature, Blackbox AI usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di Blackbox AI sia per la config specifica del progetto.
...
L'ID modello nella configurazione Blackbox AI usa il formato `provider/model-id`. Per esempio, se stai usando [Blackbox AI Zen](/docs/zen), useresti `blackbox ai/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Esegui `blackbox ai models` per vedere la lista dei modelli disponibili.
:::
...
:::tip
Hai un agente che vorresti condividere? [Invia una PR](https://github.com/anomalyco/blackbox ai).
:::
