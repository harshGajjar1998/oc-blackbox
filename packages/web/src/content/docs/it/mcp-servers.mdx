...
Puoi aggiungere strumenti esterni a Blackbox AI usando il _Model Context Protocol_, o MCP. Blackbox AI supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [Blackbox AI Config](https://www.blackbox.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/blackbox ai`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
Blackbox AI gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, Blackbox AI:
...
Se il server richiede autenticazione, Blackbox AI ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `blackbox ai mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, Blackbox AI memorizzera' i token in modo sicuro in `~/.local/share/blackbox ai/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in Blackbox AI insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione Blackbox AI come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare Blackbox AI al tuo account Sentry.
