---
title: Plugin
description: Scrivi plugin per estendere Blackbox AI.
---

I plugin ti permettono di estendere Blackbox AI agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalita', integrare servizi esterni o modificare il comportamento predefinito di Blackbox AI.
...
- `.blackbox ai/plugins/` - plugin a livello progetto
- `~/.config/blackbox ai/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/blackbox ai/node_modules/`.
...
1. Config globale (`~/.config/blackbox ai/blackbox ai.json`)
2. Config di progetto (`blackbox ai.json`)
3. Directory plugin globale (`~/.config/blackbox ai/plugins/`)
4. Directory plugin di progetto (`.blackbox ai/plugins/`)
...
Blackbox AI esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di blackbox ai per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere blackbox ai.
...
:::note
Se usi l'app desktop di Blackbox AI, puo' inviare automaticamente notifiche di sistema quando una risposta e' pronta o quando una sessione va in errore.
:::
...
Impedisci a blackbox ai di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a blackbox ai:
...
L'helper `tool` crea uno strumento personalizzato che blackbox ai puo' chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in blackbox ai insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://www.blackbox.ai/docs/sdk) per i dettagli.
