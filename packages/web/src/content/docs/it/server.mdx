---
title: Server
description: Interagisci con il server blackbox ai via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `blackbox ai serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client blackbox ai.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `blackbox ai.local` |
...
Imposta `BLACKBOX_AI_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `blackbox ai`, oppure imposta `BLACKBOX_AI_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `blackbox ai serve` sia per `blackbox ai web`.
...
Quando esegui `blackbox ai` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server blackbox ai per interagire con blackbox ai in modo programmatico.
:::

Questa architettura permette a blackbox ai di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `blackbox ai serve` per avviare un server standalone. Se la TUI di blackbox ai e gia in esecuzione, `blackbox ai serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di Blackbox AI.
...
Il server blackbox ai espone le seguenti API.
