...
Le skill degli agenti permettono a Blackbox AI di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
Blackbox AI cerca in queste posizioni:

- Config di progetto: `.blackbox ai/skills/<name>/SKILL.md`
- Config globale: `~/.config/blackbox ai/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, Blackbox AI risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.blackbox ai/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/blackbox ai/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.blackbox ai/skills/git-release/SKILL.md` cosi':
...
Blackbox AI elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `blackbox ai.json`:
...
**Per agenti integrati** (in `blackbox ai.json`):
