...
Blackbox AI 内置了两个主代理：**Build** 和 **Plan**。我们将在下面介绍它们。
...
Blackbox AI 内置了两个子代理：**General** 和 **Explore**。我们将在下面介绍它们。

---

## 内置代理

Blackbox AI 内置了两个主代理和两个子代理。
...
- 全局：`~/.config/blackbox ai/agents/`
- 项目级：`.blackbox ai/agents/`
...
如果未指定温度，Blackbox AI 将使用模型特定的默认值；大多数模型通常为 0，Qwen 模型为 0.55。
...
此路径相对于配置文件所在位置。因此它同时适用于全局 Blackbox AI 配置和项目级配置。
...
Blackbox AI 配置中的模型 ID 使用 `provider/model-id` 格式。例如，如果您使用 [Blackbox AI Zen](/docs/zen)，则可以使用 `blackbox ai/gpt-5.1-codex` 来表示 GPT 5.1 Codex。
...
:::tip
运行 `blackbox ai models` 查看可用模型列表。
:::
...
:::tip
您有想要分享的代理吗？[提交 PR](https://github.com/anomalyco/blackbox ai)。
:::
