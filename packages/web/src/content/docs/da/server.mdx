---
title: Server
description: Interager med blackbox ai-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `blackbox ai serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en blackbox ai-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `blackbox ai.local` |
...
Indstil `BLACKBOX_AI_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `blackbox ai`, eller indstil `BLACKBOX_AI_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `blackbox ai serve` og `blackbox ai web`.
...
Når du kører `blackbox ai` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug blackbox ai-serveren til at interagere med blackbox ai programmatisk.
:::

Denne arkitektur lader blackbox ai understøtte flere klienter og lader dig interagere med blackbox ai programmatisk.

Du kan køre `blackbox ai serve` for at starte en selvstændig server. Hvis du har
blackbox ai TUI kørende, vil `blackbox ai serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af Blackbox AI [IDE](/docs/ide) plugins.
...
Blackbox AI-serveren viser følgende API'er.
