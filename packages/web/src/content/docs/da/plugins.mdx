---
title: Plugins
description: Skriv dine egne plugins for at udvide Blackbox AI.
---

Plugins giver dig mulighed for at udvide Blackbox AI ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre Blackbox AIs standardadfærd.
...
- `.blackbox ai/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/blackbox ai/node_modules/`.
...
1. Global konfiguration (`~/.config/blackbox ai/blackbox ai.json`)
2. Projektkonfiguration (`blackbox ai.json`)
3. Global plugin-mappe (`~/.config/blackbox ai/plugins/`)
4. Projekt plugin bibliotek (`.blackbox ai/plugins/`)
...
Blackbox AI kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En blackbox ai SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide blackbox ai.
...
:::note
Hvis du bruger Blackbox AI desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå blackbox ai i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til blackbox ai:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som blackbox ai kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for blackbox ai sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://www.blackbox.ai/docs/sdk) for detaljer.
