...
Med Blackbox AI kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger Blackbox AI vårt eget `blackbox ai`-tema.
...
Blackbox AI kommer med flere innebygde temaer.
...
- Vil at Blackbox AI skal matche terminalens utseende
...
Blackbox AI støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/blackbox ai/themes/*.json` eller `$XDG_CONFIG_HOME/blackbox ai/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.blackbox ai/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.blackbox ai/themes/*.json`
