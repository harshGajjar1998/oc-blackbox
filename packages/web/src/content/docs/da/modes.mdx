...
:::caution
Tilstande er nu konfigureret gennem indstillingen `agent` i blackbox ai-konfigurationen. De
`mode` mulighed er nu forældet. [Learn more](/docs/agents).
:::

Tilstande i blackbox ai giver dig mulighed for at tilpasse adfærd, værktøjer og prompter til forskellige brugstilfælde.

Den kommer med to indbyggede tilstande: **build** og **plan**. Du kan tilpasse
disse eller konfigurer dine egne gennem blackbox ai-konfigurationen.
...
blackbox ai leveres med to indbyggede tilstande.
...
- `edit` - Kan ikke ændre eksisterende filer, undtagen filer placeret på `.blackbox ai/plans/*.md` for at detaljere selve planen
...
Konfigurer tilstande i din `blackbox ai.json`-konfigurationsfil:
...
- Globalt: `~/.config/blackbox ai/modes/`
...
Hvis der ikke er angivet nogen temperatur, bruger blackbox ai modelspecifikke standarder (typisk 0 for de fleste modeller, 0,55 for Qwen-modeller).
...
både den globale blackbox ai-konfiguration og den projektspecifikke konfiguration.
...
Opret tilstandsfiler i `.blackbox ai/modes/` for projektspecifikke tilstande eller `~/.config/blackbox ai/modes/` for globale tilstande:
