---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i Blackbox AI.
---

import config from "../../../../config.mjs"
export const console = config.console

Blackbox AI bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i Blackbox AI-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/blackbox ai/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i Blackbox AI-konfigurationen.
...
Blackbox AI Zen er en liste over modeller leveret af Blackbox AI-teamet, som er blevet testet og verificeret til at fungere godt med Blackbox AI. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med Blackbox AI Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg blackbox ai og gå til [blackbox.ai/auth](https://www.blackbox.ai/auth).
...
Det fungerer som alle andre udbydere i Blackbox AI og er helt valgfrit at bruge.
...
Sådan bruger du Amazon Bedrock med Blackbox AI:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører blackbox ai:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `blackbox ai.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i Blackbox AI understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at blackbox ai skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem blackbox ai-konfigurationen.
...
gemme tokenet i blackbox ai auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
Blackbox AI bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse Blackbox AI
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`blackbox ai.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [blackbox ai-gitlab-auth](https://www.npmjs.com/package/@gitlab/blackbox ai-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `blackbox ai.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med Blackbox AI:
...
Sådan bruger du Google Vertex AI med Blackbox AI:
...
2. Angiv de nødvendige miljøvariabler:
   ...
   Sæt dem mens du kører blackbox ai.
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem blackbox ai, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [blackbox ai-helicone-session](https://github.com/H2Shami/blackbox ai-helicone-session) plugin for automatisk at logge hver Blackbox AI-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver Blackbox AI-samtale opført som en separat session.
...
Du kan konfigurere blackbox ai til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere blackbox ai til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere blackbox ai til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for Blackbox AI. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/blackbox ai) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med Blackbox AI:
...
3. Kopier API-nøglen til brug i Blackbox AI.
...
6. **Vigtigt**: Før du bruger skymodeller i Blackbox AI, skal du hente modelinformationen lokalt:
...
Blackbox AI Zen er en liste over testede og verificerede modeller leveret af Blackbox AI-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>Blackbox AI Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **Blackbox AI Zen**.

   ```txt
   /connect
   ```

3. Indtast Blackbox AI API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem blackbox ai-konfigurationen.
...
5. Du kan også tilpasse dem gennem blackbox ai-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Blackbox AI:
...
Du kan også tilpasse modeller gennem blackbox ai-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med blackbox ai. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `blackbox ai.json`-filen i projektkataloget:
...
`limit`-felterne lader Blackbox AI forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `blackbox ai auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek blackbox ai-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i blackbox ai-konfigurationen.
