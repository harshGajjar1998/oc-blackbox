...
const CLI_INSTALL_DIR: &str = ".blackbox-ai/bin";
const CLI_BINARY_NAME: &str = "blackbox-ai";
...
pub fn install_cli(app: tauri::AppHandle) -> Result<String, String> {
    ...
    let temp_script = std::env::temp_dir().join("blackbox-ai-install.sh");
...
pub fn spawn_command(
    ...
    let mut cmd = if cfg!(windows) {
        if is_wsl_enabled(app) {
            ...
            let mut script = vec![
                "set -e".to_string(),
                "BIN=\"$HOME/.blackbox-ai/bin/blackbox-ai\"".to_string(),
                "if [ ! -x \"$BIN\" ]; then".to_string(),
                format!(
                    "  curl -fsSL https://blackbox.ai/install | bash -s -- --version {} --no-modify-path",
