...
- Add a guarded e2e route in backend (enabled only when a dedicated env flag is set by e2e-local runner).
  - New route file: `packages/blackbox ai/src/server/routes/e2e.ts`
  - Mount from: `packages/blackbox ai/src/server/server.ts`
  - Gate behind env flag (for example `BLACKBOX_AI_E2E=1`) so this route is never exposed in normal runs.
