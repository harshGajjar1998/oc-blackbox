...
mode: primary
hidden: true
model: blackbox ai/minimax-m2.5
color: "#44BA81"
tools:
  "*": false
  "github-triage": true
...
- `blackbox ai web` command
- The desktop app itself

**Only** add if it's specifically about the Desktop application or `blackbox ai web` view. **Do not** add for terminal, TUI, or general blackbox ai issues.
...
**Only** add if the issue mentions "zen" or "blackbox ai zen" or "blackbox ai black".

If the issue doesn't have "zen" or "blackbox ai black" in it then don't add zen label

#### core

Use for core server issues in `packages/blackbox ai/`, excluding `packages/blackbox ai/src/cli/cmd/tui/`.
...
TUI (`packages/blackbox ai/src/cli/cmd/tui/...`):
...
Core (`packages/blackbox ai/...`, excluding TUI subtree):

- thdxr for sqlite/snapshot/memory bugs and larger architectural core features
- jlongster for blackbox ai server + API feature work (tool currently remaps jlongster -> thdxr until assignable)
